...
--xcyy
--2018年5月23日
--MayanMysteryJackpotWinView.lua
local PublicConfig = require "MayanMysteryPublicConfig"
local MayanMysteryJackpotWinView = class("MayanMysteryJackpotWinView",util_require("Levels.BaseLevelDialog"))
...
function MayanMysteryJackpotWinView:initUI(params)
    --jackpot类型统一转化为小写
    ...
    if self.m_jackpotIndex == 1 then
        self:createCsbNode("MayanMystery/EpicWinView.csb")
    else
        self:createCsbNode("MayanMystery/JackpotWinView.csb")
    end

    self.m_role = util_spineCreate("MayanMystery_juese",false,true)
    self:findChild("juese"):addChild(self.m_role)

    --创建分享按钮
    self:createGrandShare()

    local guangNode1 = util_createAnimation("MayanMystery_tanban_guang.csb")
    self:findChild("guang"):addChild(guangNode1)
    guangNode1:runCsbAction("idle", true)

    if self.m_jackpotIndex == 1 then
        util_spinePlay(self.m_role, "idle_tanban2", true)

        for index = 2, 4 do
            local guangNode = util_createAnimation("MayanMystery_tanban_guang2.csb")
            self:findChild("guang"..index):addChild(guangNode)
            guangNode:runCsbAction("idle", true)
        end
    else
        util_spinePlay(self.m_role, "idle_tanban", true)
    end
    ...
    gLobalSoundManager:playSound(PublicConfig.SoundConfig["sound_MayanMystery_jackpotView_start"..self.m_jackpotIndex])
...
function MayanMysteryJackpotWinView:showView(winCoin)
...
function MayanMysteryJackpotWinView:showOver()
    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_MayanMystery_jackpotView_over)
...
function MayanMysteryJackpotWinView:jumpCoins(params)
    ...
    local jumpSound = PublicConfig.SoundConfig.sound_MayanMystery_jump_coins --跳动音效
    local jumpSoundEnd = PublicConfig.SoundConfig.sound_MayanMystery_jump_coins_end --跳动结束音效
...
function MayanMysteryJackpotWinView:clickFunc(sender)
    if not self.m_allowClick then
        return
    end

    --点击音效
    if PublicConfig.SoundConfig.sound_MayanMystery_click then
...
function MayanMysteryJackpotWinView:createGrandShare()
...
function MayanMysteryJackpotWinView:jumpCoinsFinish()
...
function MayanMysteryJackpotWinView:checkShareState()
    local bShare = false
    if nil ~= self.m_grandShare then
...
function MayanMysteryJackpotWinView:jackpotViewOver(_fun)
    if nil ~= self.m_grandShare then
...
return MayanMysteryJackpotWinView