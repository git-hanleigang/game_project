...
function PomiRespinBarView:initUI()
    ...
    self:createCsbNode("Pomi_respin_bar.csb")


    self.m_PomiRespinAction = util_createView("CodePomiSrc.PomiRespinAction")
...
function PomiRespinBarView:updateRespinLeftTimnes(times,isplay )
    ...
    elseif times == 3 then
        if isplay then
            gLobalSoundManager:playSound("PomiSounds/Pomi_respinTime_rest.mp3")
        end
