local NetSpriteLua = require("views.NetSprite")
local LottoPartySpot5 = class("LottoPartySpot5", util_require("base.BaseView"))

local imageMap = {
    {headBg = "ui/LottoParty_HeadBgMe.png", headFrame = "ui/LottoParty_HeadKuangMe.png"},
    {headBg = "ui/LottoParty_HeadBgPlayer.png", headFrame = "ui/LottoParty_HeadKuangPlayer.png"}
}

function LottoPartySpot5:initUI(data)
    self:createCsbNode("LottoParty_Spot_5.csb")
    self:updateUI(data)
end

function LottoPartySpot5:setSpotNum(_num)
    local numLab = self:findChild("BitmapFontLabel_1")
    if numLab then
        numLab:setString(_num)
    end
end

function LottoPartySpot5:resetSpot()
    self:runCsbAction(
        "idleframe",
        false,
        function()
        end,
        60
    )
end

function LottoPartySpot5:setSpotBetCoins(_coins)
    local betNum = self:findChild("m_lb_coins")
    if betNum then
        betNum:setString(util_formatCoins(_coins, 4))
    end
end

function LottoPartySpot5:isMySelf(_udid)
    if globalData.userRunData.userUdid == _udid then
        return true
    end
    return false
end

function LottoPartySpot5:openSoptItem(func)
    self:runCsbAction(
        "actionframe2",
        false,
        function()
            self:runCsbAction("idleframe3", true, nil, 60)
            if func then
                func()
            end
        end,
        60
    )
end

function LottoPartySpot5:updataSpotData(data)
    if data and data.udid ~= "" then
        local head = data.head or "0"
        local udid = data.udid
        local coins = data.coins
        local index = 1
        if self:isMySelf(udid) then
            self:runCsbAction("idleframe3", true, nil, 60)
        end
    end
end

function LottoPartySpot5:updateUI(data)
    if data and data.udid ~= "" then
        self:runCsbAction("headIdle", false, nil, 60)
        self:updataSpotData(data)
    else
        self:runCsbAction("idleframe", false, nil, 60)
    end
end

function LottoPartySpot5:startLoadFacebookHead(fbid)

end

return LottoPartySpot5
