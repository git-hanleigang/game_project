...
--smy
--2018年4月18日
--AliceWheelAction.lua

local BaseWheel = util_require("base.BaseWheel")
local AliceWheelAction = class("AliceWheelAction", BaseWheel)

function AliceWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function AliceWheelAction:changeWheelRunData( Array)
...
function AliceWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function AliceWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function AliceWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function AliceWheelAction:recvData(index)
...
function AliceWheelAction:prepareStoping()
    ...
    self.m_curA = (self.m_curV * self.m_curV - v0 * v0) / (2*self.m_stopDistance*1) --2*a*s=v*v-v0*v0
