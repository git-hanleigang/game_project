...
local AliceBonusGameJackpot = class("AliceBonusGameJackpot", util_require("base.BaseView"))

function AliceBonusGameJackpot:initUI(data)
    local isAutoScale =false
    if CC_RESOLUTION_RATIO==3 then
        isAutoScale=false
    end
    local resourceFilename = "Alice/JackpotOver.csb"
    self:createCsbNode(resourceFilename,isAutoScale)
    self.m_click = true
    self.m_JumpOver = nil 
    
    gLobalSoundManager:playSound("AliceSounds/sound_Alice_pop_window2.mp3")
    -- gLobalSoundManager:playSound("AliceSounds/sound_Alice_show_jackpot.mp3")
end

function AliceBonusGameJackpot:initViewData(coins, callBackFun)
...
function AliceBonusGameJackpot:jumpCoins(coins )
...
function AliceBonusGameJackpot:onEnter()
    
end

function AliceBonusGameJackpot:onExit()
...
function AliceBonusGameJackpot:clickFunc(sender)
...
function AliceBonusGameJackpot:closeUI( )
