...
function CashOrConkJackPotBarView:initUI(data)
    self:createCsbNode("CashOrConk_jackpot_classic.csb")
...
function CashOrConkJackPotBarView:updateSize()
    ...
    local info5 = {label = label5, sx = 0.52,sy = 0.52}

    self:updateLabelSize(info1, 365)
    self:updateLabelSize(info2, 365)
    self:updateLabelSize(info3, 365)
    self:updateLabelSize(info4, 365)
    self:updateLabelSize(info5, 365)
end
