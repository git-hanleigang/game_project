local CashOrConkDF12WheelAction = class("CashOrConkDF12WheelAction", util_require("base.BaseWheel"))

function CashOrConkDF12WheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function CashOrConkDF12WheelAction:changeWheelRunData( Array)
...
function CashOrConkDF12WheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function CashOrConkDF12WheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function CashOrConkDF12WheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function CashOrConkDF12WheelAction:recvData(index)
...
return CashOrConkDF12WheelAction