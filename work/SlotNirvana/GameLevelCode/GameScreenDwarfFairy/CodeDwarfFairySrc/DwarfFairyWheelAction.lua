...
--smy
--2018年4月18日
--DwarfFairyWheelAction.lua


local DwarfFairyWheelAction = class("DwarfFairyWheelAction", util_require("base.BaseWheel"))

function DwarfFairyWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function DwarfFairyWheelAction:changeWheelRunData( Array)
...
function DwarfFairyWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function DwarfFairyWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function DwarfFairyWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function DwarfFairyWheelAction:recvData(index)
