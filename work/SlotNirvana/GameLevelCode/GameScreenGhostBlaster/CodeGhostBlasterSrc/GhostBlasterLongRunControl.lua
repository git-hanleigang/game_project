...
local GhostBlasterLongRunControl = class("GhostBlasterLongRunControl")

function GhostBlasterLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function GhostBlasterLongRunControl:getUsingReels()
    return self.usingReels
end

function GhostBlasterLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function GhostBlasterLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
    ...
    for index = 1, #_LegitimateCol do
        ...
        local isHave = false
        if symbolNum == _LegitimateNum then
            info.startCol = iCol
            info.longRun = true
        end
...
function GhostBlasterLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function GhostBlasterLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function GhostBlasterLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function GhostBlasterLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function GhostBlasterLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function GhostBlasterLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function GhostBlasterLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function GhostBlasterLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function GhostBlasterLongRunControl:setLongRunLenAndStates()
    ...
    local getRunlen = function(_lastColLens, _colHeight, _columnData,_norAdd)
        local reelCount = (self.m_machine.m_configData.p_reelLongRunTime * self.m_machine.m_configData.p_reelLongRunSpeed) / _colHeight
        if _norAdd then
            -- 不参与快滚的列只滚动一秒
            reelCount =  self.m_machine.m_configData.p_reelLongRunSpeed / _colHeight
        end
    ...
    for iCol = startCol, self.m_machine.m_iReelColumnNum do
        ...
        if iCol == startCol then
            ...
            if startCol == 1 then
                runLen = getRunlen(0, colHeight, columnData)
            end
        elseif iCol > startCol and iCol <= endCol then
            ...
            if table_vIn(unlegitimateCol,iCol) then
                runLen = getRunlen(lastColLens, colHeight, columnData,true)
                self.m_machine.m_reelRunInfo[iCol]:setReelLongRun(false)
            else
                runLen = getRunlen(lastColLens, colHeight, columnData)
                self.m_machine.m_reelRunInfo[iCol]:setReelLongRun(true)
            end
            
        elseif iCol > endCol then
            local reelRunData = self.m_machine.m_reelRunInfo[iCol - 1]
            local diffLen = self.m_machine.m_reelRunInfo[2]:getInitReelRunLen() - self.m_machine.m_reelRunInfo[1]:getInitReelRunLen()
            local lastRunLen = reelRunData:getReelRunLen()
            runLen = lastRunLen + diffLen
            self.m_machine.m_reelRunInfo[iCol]:setReelLongRun(false)
        end
...
function GhostBlasterLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function GhostBlasterLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function GhostBlasterLongRunControl:clearCurLongRunData()
...
function GhostBlasterLongRunControl:setFirstColBeginLongRun()
...
return GhostBlasterLongRunControl
