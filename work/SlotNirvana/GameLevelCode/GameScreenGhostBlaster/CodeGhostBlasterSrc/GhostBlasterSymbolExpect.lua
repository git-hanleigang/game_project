--[[
    ...
    关卡创建:
        self.m_symbolExpectCtr = util_createView("GhostBlasterSrc.GhostBlasterSymbolExpect", self)
    以下接口需要在关卡内的直接重写:
        GhostBlasterSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        GhostBlasterSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        GhostBlasterSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        GhostBlasterSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        GhostBlasterSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local GhostBlasterSymbolExpect = class("GhostBlasterSymbolExpect")

function GhostBlasterSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function GhostBlasterSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function GhostBlasterSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function GhostBlasterSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function GhostBlasterSymbolExpect:isReelRunSymbolType(_symbolType)
...
function GhostBlasterSymbolExpect:MachineOneReelDownCall(_iCol)
    ...
    if _iCol == self.m_machine.m_iReelColumnNum and 0 ~= self.m_reelRunCol then
        self.m_reelRunCol = 0
        local isTriggerFree = self.m_machine:getCurIsTriggerFree()
        if isTriggerFree then
            --停止最后二列之前的待触发
            self:stopExpectAnim(self.m_machine.m_iReelColumnNum-1)
        else
            --停止所有期待
            self:stopExpectAnim()
        end
...
--快滚检测 
function GhostBlasterSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function GhostBlasterSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function GhostBlasterSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function GhostBlasterSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
...
function GhostBlasterSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function GhostBlasterSymbolExpect:stopExpectAnim(_maxCol)
    local maxCol = _maxCol or self.m_machine.m_iReelColumnNum
    local maxRow = self.m_machine.m_iReelRowNum
...
--播放循环idle
function GhostBlasterSymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return GhostBlasterSymbolExpect