--[[
    ...
    关卡创建:
        self.m_symbolExpectCtr = util_createView("CodeStarryFestSrc.GeminiJourneySymbolExpect", self)
    以下接口需要在关卡内的直接重写:
        GeminiJourneySymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        GeminiJourneySymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        GeminiJourneySymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        GeminiJourneySymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        GeminiJourneySymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local GeminiJourneySymbolExpect = class("GeminiJourneySymbolExpect")

function GeminiJourneySymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function GeminiJourneySymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function GeminiJourneySymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function GeminiJourneySymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER,
    }
end
function GeminiJourneySymbolExpect:isReelRunSymbolType(_symbolType)
...
function GeminiJourneySymbolExpect:MachineOneReelDownCall(_iCol)
    performWithDelay(self.m_machine.m_scWaitNode, function()
        if self:getNextReelLongRunState(_iCol) and _iCol >= self.m_machine.m_quickRunSymbolAndCol.SCATTER_MIN_COL then
...
--快滚检测 
function GeminiJourneySymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function GeminiJourneySymbolExpect:isLoopIdleSymbol(_symbolType)
...
function GeminiJourneySymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function GeminiJourneySymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
...
function GeminiJourneySymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function GeminiJourneySymbolExpect:stopExpectAnim()
...
--播放循环idle
function GeminiJourneySymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return GeminiJourneySymbolExpect