...
local GeminiJourneyLongRunControl = class("GeminiJourneyLongRunControl")

function GeminiJourneyLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function GeminiJourneyLongRunControl:getUsingReels()
    return self.usingReels
end

function GeminiJourneyLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function GeminiJourneyLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function GeminiJourneyLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
    ...
    if _symbolTypes[1] == TAG_SYMBOL_TYPE.SYMBOL_SCATTER then
        legitimateNum = 2
    elseif _symbolTypes[1] == self.m_machine.SYMBOL_SCORE_BONUS_1 then
        legitimateNum = 4
    end
...
function GeminiJourneyLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function GeminiJourneyLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function GeminiJourneyLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function GeminiJourneyLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function GeminiJourneyLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function GeminiJourneyLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function GeminiJourneyLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
    local endLongRunInfos = {}
    ...
    for index = 1, #_longRunConfigs do
        ...
        if table_length(infos) > 0 and infos.startCol and infos.startCol > 1 then
            if symbolTypes[1] == TAG_SYMBOL_TYPE.SYMBOL_SCATTER then
                self.m_machine.m_quickRunSymbolAndCol.SCATTER_MIN_COL = infos.startCol
            elseif symbolTypes[1] == self.m_machine.SYMBOL_SCORE_BONUS_1 then
                self.m_machine.m_quickRunSymbolAndCol.BONUS_MIN_COL = infos.startCol
            end
    ...
    -- 取最小列快滚列
    table.sort(longRunInfos, function(a, b)
        if a.startCol and b.startCol then
            return a.startCol < b.startCol
        end
    end)

    endLongRunInfos[1] = longRunInfos[1]

    local isHaveAllLongRun = false
    for index = 1, #endLongRunInfos do
        local longRunInfo = endLongRunInfos[index]
...
function GeminiJourneyLongRunControl:setLongRunLenAndStates()
...
function GeminiJourneyLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function GeminiJourneyLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function GeminiJourneyLongRunControl:clearCurLongRunData()
...
function GeminiJourneyLongRunControl:setFirstColBeginLongRun()
...
return GeminiJourneyLongRunControl
