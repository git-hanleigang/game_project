...
--smy
--2018年4月18日
--EgyptWheelAction.lua


local EgyptWheelAction = class("EgyptWheelAction", util_require("base.BaseWheel"))

function EgyptWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function EgyptWheelAction:changeWheelRunData( Array)
...
function EgyptWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function EgyptWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function EgyptWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function EgyptWheelAction:recvData(index)
