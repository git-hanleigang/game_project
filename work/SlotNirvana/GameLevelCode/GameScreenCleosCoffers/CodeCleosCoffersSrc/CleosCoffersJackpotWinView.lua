...
--xcyy
--2018年5月23日
--CleosCoffersJackpotWinView.lua
local PublicConfig = require "CleosCoffersPublicConfig"
local CleosCoffersJackpotWinView = class("CleosCoffersJackpotWinView",util_require("Levels.BaseLevelDialog"))
...
function CleosCoffersJackpotWinView:initUI(params)
    --jackpot类型统一转化为小写
    ...
    self:createCsbNode("CleosCoffers/JackpotWinView.csb")

    local spineSkinName = {"grand", "mega", "major", "minor", "mini"}
    self.m_jackpotSpine = util_spineCreate("JackpotWinView_spine",true,true)
    self:findChild("Node_spine"):addChild(self.m_jackpotSpine)
    self.m_jackpotSpine:setSkin(spineSkinName[jackpotIndex])

    local numAni = util_createAnimation("CleosCoffers/JackpotWinView_3.csb")
    util_spinePushBindNode(self.m_jackpotSpine, "shuzi", numAni)
    local btnAni = util_createAnimation("CleosCoffers/JackpotWinView_2.csb")
    util_spinePushBindNode(self.m_jackpotSpine, "anniu", btnAni)
    self.m_btnNode = btnAni:findChild("Button")
    self.m_btnNode:setTouchEnabled(false)
    self.m_label = numAni:findChild("m_lb_coins")

    self.m_btnNode:addTouchEventListener(function(sender, eventType)
...
function CleosCoffersJackpotWinView:showView(winCoin)
    self:runCsbAction("start",false,function()
        self:runCsbAction("idle",true)
    end)

    util_spinePlay(self.m_jackpotSpine, "start", false)
    util_spineEndCallFunc(self.m_jackpotSpine, "start", function()
        self.m_btnNode:setTouchEnabled(true)
        self.m_allowClick = true
        util_spinePlay(self.m_jackpotSpine, "idle", true)
    end)

    self:jumpCoins({label = self.m_label, endCoins = winCoin,  maxWidth = 1100})
end

--[[
    关闭界面
]]
function CleosCoffersJackpotWinView:showOver()
    self.m_allowClick = false
    gLobalSoundManager:playSound(PublicConfig.SoundConfig.Music_Fg_OverOver)
    util_spinePlay(self.m_jackpotSpine, "over", false)
    self.m_machine:closeJackpotWinCallFunc()
...
function CleosCoffersJackpotWinView:jumpCoins(params)
    ...
    local jumpSound = PublicConfig.SoundConfig.sound_CleosCoffers_jump_coins --跳动音效
    local jumpSoundEnd = PublicConfig.SoundConfig.sound_CleosCoffers_jump_coins_end --跳动结束音效
...
function CleosCoffersJackpotWinView:clickFunc(sender)
...
function CleosCoffersJackpotWinView:createGrandShare()
...
function CleosCoffersJackpotWinView:jumpCoinsFinish()
...
function CleosCoffersJackpotWinView:checkShareState()
    local bShare = false
    if nil ~= self.m_grandShare then
...
function CleosCoffersJackpotWinView:jackpotViewOver(_fun)
    if nil ~= self.m_grandShare then
...
return CleosCoffersJackpotWinView