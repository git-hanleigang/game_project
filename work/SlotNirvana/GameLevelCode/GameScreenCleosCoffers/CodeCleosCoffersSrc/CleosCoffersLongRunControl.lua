...
local CleosCoffersLongRunControl = class("CleosCoffersLongRunControl")

function CleosCoffersLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function CleosCoffersLongRunControl:getUsingReels()
    return self.usingReels
end

function CleosCoffersLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function CleosCoffersLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function CleosCoffersLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function CleosCoffersLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CleosCoffersLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CleosCoffersLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function CleosCoffersLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function CleosCoffersLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function CleosCoffersLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function CleosCoffersLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function CleosCoffersLongRunControl:setLongRunLenAndStates()
...
function CleosCoffersLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function CleosCoffersLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function CleosCoffersLongRunControl:clearCurLongRunData()
...
function CleosCoffersLongRunControl:setFirstColBeginLongRun()
...
return CleosCoffersLongRunControl
