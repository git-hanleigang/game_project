local LuxeVegasSymbol = class("LuxeVegasSymbol",util_require("Levels.BaseLevelDialog"))


-- 构造函数
function LuxeVegasSymbol:ctor()
    LuxeVegasSymbol.super.ctor(self)
...
function LuxeVegasSymbol:initDatas(_m_machine)
    self.m_machine   = _m_machine
end

function LuxeVegasSymbol:changeSymbolCcb(_symbolType)
...
function LuxeVegasSymbol:createAnimNode()
...
function LuxeVegasSymbol:upDateAnimNode()
...
function LuxeVegasSymbol:runAnim(animName,loop,func)
    self:createAnimNode()
...
function LuxeVegasSymbol:getNodeSpine()
    return self.m_animNode
end

function LuxeVegasSymbol:getCcbProperty(propName)
...
-- 获取动画持续时间
--
function LuxeVegasSymbol:getAnimDurationTime(animName)
