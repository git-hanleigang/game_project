--island
--2018年4月12日
--LuxeVegasSmallWheelVertical.lua
--
local LuxeVegasPublicConfig = require "LuxeVegasPublicConfig"
local LuxeVegasSmallWheelVertical = class("LuxeVegasSmallWheelVertical",util_require("Levels.BaseReel.BaseReelNode"))
...
function LuxeVegasSmallWheelVertical:ctor(params)
    LuxeVegasSmallWheelVertical.super.ctor(self,params)
    self.m_machine = params.parentData._machine
    self.m_wheelNode = params._wheelNode
...
function LuxeVegasSmallWheelVertical:createClipNode()
...
function LuxeVegasSmallWheelVertical:startSchedule()
    --先加速再匀速后减速
...
function LuxeVegasSmallWheelVertical:checkChangeSpeed(dt)
...
function LuxeVegasSmallWheelVertical:getNextSymbolType()
...
function LuxeVegasSmallWheelVertical:reloadRollNode(rollNode,rowIndex)

    self:removeSymbolByRowIndex(rowIndex)

    local symbolType = self:getNextSymbolType()

    if self.m_wheelNode.m_initShow and rowIndex <= 5 then
        local specialNodeList = {5, 302, 10, 201, 2}
        if self.m_machine.m_gamePlayMul == self.m_machine.M_ENUM_TYPE.BASE or self.m_machine.m_gamePlayMul == self.m_machine.M_ENUM_TYPE.FREE_1 then
            specialNodeList = {5, 302, 10, 201, 2}
        elseif self.m_machine.m_gamePlayMul == self.m_machine.M_ENUM_TYPE.FREE_2 then
            specialNodeList = {8, 304, 25, 202, 5}
        elseif self.m_machine.m_gamePlayMul == self.m_machine.M_ENUM_TYPE.FREE_3 then
            specialNodeList = {8, 304, 50, 202, 5}
        end
        local specialSymbolType = specialNodeList[rowIndex]
        if specialSymbolType then
            symbolType = specialSymbolType
        end
    end
...
function LuxeVegasSmallWheelVertical:updateRollNodePos(offset)
...
function LuxeVegasSmallWheelVertical:scaleRollNode(rollNode,rowIndex)
    local scale = 1
...
function LuxeVegasSmallWheelVertical:removeAllSymbol( )
    if self.m_machine.m_isRemoveWheelSymbol then
        self:forEachRollNode(function(rollNode,bigRollNode,iRow)
            if rollNode then
...
function LuxeVegasSmallWheelVertical:removeSymbolFromRollNode(rollNode)
...
return LuxeVegasSmallWheelVertical
