--[[
    ...
    以下接口需要在关卡内的直接重写:
        LuxeVegasSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        LuxeVegasSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        LuxeVegasSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        LuxeVegasSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        LuxeVegasSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local LuxeVegasSymbolExpect = class("LuxeVegasSymbolExpect")

function LuxeVegasSymbolExpect:initData_(_machine)
    ...
    self.m_idleSymbolTypeList     = {                     --循环idle类型的图标列表
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER,
        self.m_machine.SYMBOL_SCORE_BONUS,
        self.m_machine.SYMBOL_SCORE_WHEEL_BONUS,
        self.m_machine.SYMBOL_SCORE_BONUS_FREE_1,
        self.m_machine.SYMBOL_SCORE_BONUS_FREE_2,
        self.m_machine.SYMBOL_SCORE_BONUS_FREE_3,
    }
end

-- 接口在关卡内直接重写
--[[
function LuxeVegasSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function LuxeVegasSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function LuxeVegasSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function LuxeVegasSymbolExpect:isReelRunSymbolType(_symbolType)
...
function LuxeVegasSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function LuxeVegasSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function LuxeVegasSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function LuxeVegasSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function LuxeVegasSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
...
function LuxeVegasSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function LuxeVegasSymbolExpect:stopExpectAnim()
...
--播放循环idle
function LuxeVegasSymbolExpect:playSymbolIdleAnim(_slotsNode)
    ...
    elseif _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS then
        _slotsNode:runAnim("idleframe2", true)
    elseif _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_WHEEL_BONUS then
        _slotsNode:runAnim("idleframe2", true)
    elseif _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS_FREE_1 or
     _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS_FREE_2 or
     _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS_FREE_3 then
        _slotsNode:runAnim("idleframe", true)
    end
end

return LuxeVegasSymbolExpect