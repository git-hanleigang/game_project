--island
--2018年4月12日
--LuxeVegasWheelVertical.lua
--
local LuxeVegasPublicConfig = require "LuxeVegasPublicConfig"
local LuxeVegasWheelVertical = class("LuxeVegasWheelVertical",util_require("Levels.BaseReel.BaseReelNode"))
...
local ACC_SPEED_TIMES = 2.5         --加速时间 单位s(秒)
local DECELER_SPEED_TIMES = 5      --减速时间 单位s(秒)
local HIGH_SPEED_TIME = 2.5         --匀速时间
local MAX_SPEED = 6000
local MIN_SPEED = 120

function LuxeVegasWheelVertical:ctor(params)
    LuxeVegasWheelVertical.super.ctor(self,params)
...
function LuxeVegasWheelVertical:createClipNode()
...
function LuxeVegasWheelVertical:startMove(func, _isClickStart)
    local isClickStart = _isClickStart
    if isClickStart then
        self:startSchedule(isClickStart)
    else
        LuxeVegasWheelVertical.super.startMove(self, func)
    end
end
...
function LuxeVegasWheelVertical:startSchedule(_isClickStart)
    local isClickStart = _isClickStart
    if isClickStart then
        --先加速再匀速后减速
...
function LuxeVegasWheelVertical:checkChangeSpeed(dt)
...
function LuxeVegasWheelVertical:getNextSymbolType()
...
function LuxeVegasWheelVertical:reloadRollNode(rollNode,rowIndex)
...
function LuxeVegasWheelVertical:updateRollNodePos(offset)
...
function LuxeVegasWheelVertical:scaleRollNode(rollNode,rowIndex)
    local scale = 1
...
function LuxeVegasWheelVertical:removeSymbolFromRollNode(rollNode)
