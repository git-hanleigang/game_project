...
local LuxeVegasLongRunControl = class("LuxeVegasLongRunControl")

function LuxeVegasLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function LuxeVegasLongRunControl:getUsingReels()
    return self.usingReels
end

function LuxeVegasLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function LuxeVegasLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function LuxeVegasLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function LuxeVegasLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function LuxeVegasLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function LuxeVegasLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function LuxeVegasLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function LuxeVegasLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function LuxeVegasLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function LuxeVegasLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function LuxeVegasLongRunControl:setLongRunLenAndStates()
...
function LuxeVegasLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function LuxeVegasLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function LuxeVegasLongRunControl:clearCurLongRunData()
...
function LuxeVegasLongRunControl:setFirstColBeginLongRun()
...
return LuxeVegasLongRunControl
