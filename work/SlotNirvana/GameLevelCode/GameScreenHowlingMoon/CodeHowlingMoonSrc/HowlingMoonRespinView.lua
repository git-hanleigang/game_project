...
function HowlingMoonView:createRsOverOneActionSymbol(endNode,actionName,actParent )
      ...
      if endNode.p_rowIndex <= unlockedLines then
            ...
            if endNode.m_lastNode.p_symbolType == self.m_machine.SYMBOL_FIX_GRAND then
...
function HowlingMoonView:createOneActionSymbol(endNode,actionName,actParent,_ccbName )
      ...
      if endNode.p_rowIndex <= unlockedLines then
            ...
            node:playAction(actionName,true,func)  
            if endNode.p_symbolType == self.m_machine.SYMBOL_FIX_GRAND then
                  node:findChild("node_grand"):setVisible(self.m_machine.m_jackpot_status == "Normal")
                  node:findChild("node_mega"):setVisible(self.m_machine.m_jackpot_status == "Mega")
                  node:findChild("node_super"):setVisible(self.m_machine.m_jackpot_status == "Super")
            end
            ...
            if actParent then
                  local worldPos = fatherNode:getParent():convertToWorldSpace(cc.p(fatherNode:getPositionX(), fatherNode:getPositionY()))
                  local pos = actParent:convertToNodeSpace(cc.p(worldPos.x,worldPos.y))
                  actParent:addChild(node , REEL_SYMBOL_ORDER.REEL_ORDER_1 - endNode.p_rowIndex)
                  node:setPosition(pos)
            else
                  local worldPos = fatherNode:getParent():convertToWorldSpace(cc.p(fatherNode:getPositionX(), fatherNode:getPositionY()))
                  local pos = self:convertToNodeSpace(cc.p(worldPos.x,worldPos.y))
      ...
      local rowCount = 0
        ...
        if iRow ~= nil and iRow <= rowCount and iCol ~= nil and (symbolNode.m_isLastSymbol == true  or rsOver ) then 
            --获取分数
                    ...
                    if score then
                        ...
                        if type(score) ~= "string" then
                            ...
                            local label = node:findChild("m_lb_score")
            ...
            local score =  self.m_machine:randomDownRespinSymbolScore(symbolNode.p_symbolType)
                  ...
                  score = score * lineBet
                  score = util_formatCoins(score, 3)
                  local label =  node:getChildByName("m_lb_score")
                  if type(label.setString) == "function" then
                        label:setString(score)
                  end
            end
...
function HowlingMoonView:updateLockNum()
      local lcokNode = self:getUsefulFixSlotsNode( )
      local lightNum = 0
      local lockedSymbols = #lcokNode
...
function HowlingMoonView:startMove()
      -- gLobalSoundManager:playSound("HowlingMoonSounds/sound_HowlingMoon_reel_run_rs.mp3")
