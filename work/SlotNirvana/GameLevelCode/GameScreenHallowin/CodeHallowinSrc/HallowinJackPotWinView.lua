...
--island
--2018年4月12日
--HallowinJackPotWinView.lua
---- respin 玩法结算时中 mini mijor等提示界面
local HallowinJackPotWinView = class("HallowinJackPotWinView", util_require("base.BaseView"))

function HallowinJackPotWinView:initUI(data)
    self.m_click = false

    local resourceFilename = "Hallowin/JackpotLayer.csb"
    self:createCsbNode(resourceFilename)

end

function HallowinJackPotWinView:initViewData(index,coins,callBackFun)
    self.m_index = index
...
function HallowinJackPotWinView:onEnter()
end

function HallowinJackPotWinView:onExit()
    
end

function HallowinJackPotWinView:clickFunc(sender)
