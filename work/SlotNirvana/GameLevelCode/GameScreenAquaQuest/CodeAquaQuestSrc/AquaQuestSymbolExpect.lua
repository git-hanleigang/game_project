...
local AquaQuestSymbolExpect = class("AquaQuestSymbolExpect")

function AquaQuestSymbolExpect:initData_(params)
...
function AquaQuestSymbolExpect:MachineOneReelDownCall(colIndex,specialList)
    if colIndex == 1 then
        self.m_bulingEndSymbols = {}
    end
...
function AquaQuestSymbolExpect:checkIsExpectSymbol(symbolType)
...
function AquaQuestSymbolExpect:MachineSymbolBulingEndCall(symbolNode)
...
function AquaQuestSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
function AquaQuestSymbolExpect:getAniName(symbolType,isIdle)
    for k,data in pairs(self.m_symbolList) do
        local list = data.symbolTypeList
        for index,triggerType in pairs(list) do
            if triggerType == symbolType then
...
--播放循环idle
function AquaQuestSymbolExpect:playSymbolIdleAnim(_slotsNode)
    --没播完落地的等播完落地再播idle
    local symbolTag = self.m_machine:getNodeTag(_slotsNode.p_cloumnIndex, _slotsNode.p_rowIndex, SYMBOL_NODE_TAG)
