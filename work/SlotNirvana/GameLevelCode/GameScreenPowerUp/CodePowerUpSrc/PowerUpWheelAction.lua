--轮盘转动界面
--PowerUpWheelAction.lua

local PowerUpWheelAction = class("PowerUpWheelAction", util_require("base.BaseWheel"))

function PowerUpWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function PowerUpWheelAction:changeWheelRunData( Array)
...
function PowerUpWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function PowerUpWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function PowerUpWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end

function PowerUpWheelAction:recvData(index,dirType)
