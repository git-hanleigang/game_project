...
local DemonessFairLongRunControl = class("DemonessFairLongRunControl")

function DemonessFairLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function DemonessFairLongRunControl:getUsingReels()
    return self.usingReels
end

function DemonessFairLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function DemonessFairLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function DemonessFairLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function DemonessFairLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function DemonessFairLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function DemonessFairLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function DemonessFairLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function DemonessFairLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function DemonessFairLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function DemonessFairLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
    ...
    for index = 1, #longRunInfos do
        ...
        if longRunInfo.longRun then
            -- 本轮是否有快滚
            if longRunInfo.isScatter then
                longRunData.longRun = true
                longRunData.isScatter = longRunInfo.isScatter
            else
                -- bonus在base下没快滚
...
function DemonessFairLongRunControl:setLongRunLenAndStates()
...
function DemonessFairLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function DemonessFairLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function DemonessFairLongRunControl:clearCurLongRunData()
...
function DemonessFairLongRunControl:setFirstColBeginLongRun()
...
return DemonessFairLongRunControl
