...
local DragonParadeDialog = class("DragonParadeDialog", util_require("Levels.BaseDialog"))

--重写 修改只有不是start可以点击
function DragonParadeDialog:clickFunc(sender)
...
function DragonParadeDialog:showStart()
    ...
    DragonParadeDialog.super.showStart(self)
end

function DragonParadeDialog:showidle()
    local button = self:findChild("Button_1")
    if button then
        if button and button.setTouchEnabled then
    ...
    DragonParadeDialog.super.showidle(self)
end

return DragonParadeDialog
