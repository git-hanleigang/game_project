...
local DragonParadeRespinNode = class("DragonParadeRespinNode",util_require("Levels.RespinNode"))

local NODE_TAG = 10
local MOVE_SPEED = 1500     --滚动速度 像素/每秒
local RES_DIS = 20
-- 构造函数
function DragonParadeRespinNode:ctor()
    DragonParadeRespinNode.super.ctor(self)
    self.m_isQuick = false
end


--裁切区域
function DragonParadeRespinNode:initClipNode(clipNode,opacity)
...
--裁切遮罩透明度
function DragonParadeRespinNode:initClipOpacity(opacity)
    if opacity and opacity>0 then
    end
end

--放入首节点
function DragonParadeRespinNode:setFirstSlotNode(node)
...
--刷新坐标
function DragonParadeRespinNode:updateBaseNodePos()
...
--重置节点坐标
function DragonParadeRespinNode:baseResetNodePos()
    if self.m_baseFirstNode then
...
--子类可以重写 读取配置
function DragonParadeRespinNode:initRunningData()
...
--设置快滚状态
function DragonParadeRespinNode:changeRunQuick(isQuick)
    self.m_isQuick = isQuick
end
--设置滚动速度
function DragonParadeRespinNode:changeRunSpeed(isQuick)
    if isQuick then
...
--设置回弹距离
function DragonParadeRespinNode:changeResDis(isQuick)
    if isQuick then
...
--获取回弹动作序列
function DragonParadeRespinNode:getBaseResAction(startPos)
    local timeDown = 0
...
--执行回弹动作
function DragonParadeRespinNode:runBaseResAction()
...
function DragonParadeRespinNode:addTipNode(parentNode,zOrder)
    self.m_edgeTip = util_createAnimation("DragonParade_zjk.csb")
...
function DragonParadeRespinNode:showTip(type)
    self.m_edgeTip:setVisible(true)
    -- self.m_edgeTip:runCsbAction("actionframe", true)
end

function DragonParadeRespinNode:hideTip()
    self.m_edgeTip:setVisible(false)
end

return DragonParadeRespinNode