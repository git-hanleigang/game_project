...
--smy
--2018年4月18日
--AladdinWheelAction.lua


local AladdinWheelAction = class("AladdinWheelAction", util_require("base.BaseWheel"))

function AladdinWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function AladdinWheelAction:resetData()
    self.m_status = self.ACTION_READY
    self.m_isWheelData = false
    self.m_targetDistance = 0
    self.m_moveDistance = 0
...
function AladdinWheelAction:changeWheelRunData( Array)
...
function AladdinWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function AladdinWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function AladdinWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function AladdinWheelAction:recvData(index)
