...
--smy
--2018年4月18日
--CrazyBombWheelAction.lua


local CrazyBombWheelAction = class("CrazyBombWheelAction", util_require("base.BaseWheel"))

function CrazyBombWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function CrazyBombWheelAction:changeWheelRunData( Array)
...
function CrazyBombWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function CrazyBombWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function CrazyBombWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function CrazyBombWheelAction:recvData(index)
