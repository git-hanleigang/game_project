...
local CatchMonstersLongRunControl = class("CatchMonstersLongRunControl")

function CatchMonstersLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function CatchMonstersLongRunControl:getUsingReels()
    return self.usingReels
end

function CatchMonstersLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function CatchMonstersLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function CatchMonstersLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
    ...
    local unlegitimateCol = {}
    for _col, _isSlowRun in ipairs(self.m_machine.m_isColReelsSlowRun) do
        if _isSlowRun then
            table.insert(unlegitimateCol, _col)
        end
    end
    info.unlegitimateCol = unlegitimateCol
    return info
end

function CatchMonstersLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CatchMonstersLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CatchMonstersLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function CatchMonstersLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function CatchMonstersLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function CatchMonstersLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function CatchMonstersLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function CatchMonstersLongRunControl:setLongRunLenAndStates()
    ...
    for iCol = startCol, self.m_machine.m_iReelColumnNum do
        ...
        if self.m_machine.m_isLongRun then
            local reelRunDatas = {32, 46, 60, 74, 88}
            runLen = reelRunDatas[iCol]
        end
...
function CatchMonstersLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function CatchMonstersLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function CatchMonstersLongRunControl:clearCurLongRunData()
...
function CatchMonstersLongRunControl:setFirstColBeginLongRun()
    for i = 1, self.m_machine.m_iReelColumnNum do
        ...
        if self.m_machine.m_isLongRun then
            parentData.moveSpeed = self.m_machine.m_configData.p_reelMoveSpeed
        else
...
function CatchMonstersLongRunControl:getScatterNumByCol(_col)
    local scatterNum = 0
    for iRow = 1, self.m_machine.m_iReelRowNum do
        for iCol = 1, _col do
...
return CatchMonstersLongRunControl
