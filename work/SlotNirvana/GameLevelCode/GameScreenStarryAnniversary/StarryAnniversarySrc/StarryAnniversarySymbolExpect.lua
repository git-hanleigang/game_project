--[[
    ...
    关卡创建:
        self.m_symbolExpectCtr = util_createView("CodeStarryAnniversarySrc.StarryAnniversarySymbolExpect", self)
    以下接口需要在关卡内的直接重写:
        StarryAnniversarySymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        StarryAnniversarySymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        StarryAnniversarySymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        StarryAnniversarySymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        StarryAnniversarySymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local StarryAnniversarySymbolExpect = class("StarryAnniversarySymbolExpect")

function StarryAnniversarySymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function StarryAnniversarySymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function StarryAnniversarySymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function StarryAnniversarySymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function StarryAnniversarySymbolExpect:isReelRunSymbolType(_symbolType)
...
function StarryAnniversarySymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function StarryAnniversarySymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function StarryAnniversarySymbolExpect:isLoopIdleSymbol(_symbolType)
...
function StarryAnniversarySymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function StarryAnniversarySymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function StarryAnniversarySymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function StarryAnniversarySymbolExpect:stopExpectAnim()
...
--播放循环idle
function StarryAnniversarySymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return StarryAnniversarySymbolExpect