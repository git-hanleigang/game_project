...
--smy
--2018年4月18日
--FourInOneBonus_WheelAction.lua


local FourInOneBonus_WheelAction = class("FourInOneBonus_WheelAction", util_require("base.BaseWheel"))

function FourInOneBonus_WheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function FourInOneBonus_WheelAction:changeWheelRunData( Array)
...
function FourInOneBonus_WheelAction:updateEndCallFunc( func )
    self.m_func = func
end



function FourInOneBonus_WheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function FourInOneBonus_WheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function FourInOneBonus_WheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function FourInOneBonus_WheelAction:recvData(index)
...
return FourInOneBonus_WheelAction