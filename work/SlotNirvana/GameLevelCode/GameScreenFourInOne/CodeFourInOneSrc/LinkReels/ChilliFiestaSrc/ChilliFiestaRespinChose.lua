...
function ChilliFiestaRespinChose:initUI(data,callback)
    local resourceFilename = "LinkReels/ChilliFiestaLink/4in1_ChilliFiesta_ExtraTimes.csb"
    ...
    gLobalSoundManager:playSound("FourInOneSounds/ChilliFiestaSounds/music_ChilliFiesta_respinChoose.mp3")
end
...
function ChilliFiestaRespinChose:clickFunc(sender)
    local name = sender:getName()
     ...
     gLobalSoundManager:playSound("FourInOneSounds/ChilliFiestaSounds/music_ChilliFiesta_respinChooseClick.mp3")
    ...
    for i=1,#temp do
        ...
        if i == type then
            ...
            self:changeSprite(self.m_chooseNumList[i],"#Common/ChiliFiesta_respin_"..num..".png")
            if num > 1 then
                self:changeSprite(self.m_chooseSpinList[i],"#Common/ChiliFiesta_respin_spins"..".png")
            else
                self:changeSprite(self.m_chooseSpinList[i],"#Common/ChiliFiesta_respin_spin"..".png")
            end
    ...
    performWithDelay(self,function()
        for i=1,#temp do
            ...
            if i ~= type then
                ...
                self.m_chooseBgList[i]:setVisible(false)
                self.m_chooseBgSmallList[i]:setVisible(false)
                self:changeSprite(self.m_chooseNumList[i],"#Common/ChiliFiesta_respin_"..num..num..".png")
                if num > 1 then
                    self:changeSprite(self.m_chooseSpinList[i],"#Common/ChiliFiesta_respin_spinsUn2"..".png")
                else
                    self:changeSprite(self.m_chooseSpinList[i],"#Common/ChiliFiesta_respin_spinUn2"..".png")
                end
...
function ChilliFiestaRespinChose:initEffect()
    self.m_effect1 = util_createAnimation("LinkReels/ChilliFiestaLink/4in1_ChilliFiesta_ExtraTimes_xuanzhong.csb")
    self.m_effect1:playAction("idle")
    self:findChild("effNode_1"):addChild(self.m_effect1)
    self.m_effect1:setVisible(false)
    self.m_effect2 = util_createAnimation("LinkReels/ChilliFiestaLink/4in1_ChilliFiesta_ExtraTimes_xuanzhong.csb")
    self.m_effect2:playAction("idle")
    self:findChild("effNode_2"):addChild(self.m_effect2)
    self.m_effect2:setVisible(false)
    self.m_effect3 = util_createAnimation("LinkReels/ChilliFiestaLink/4in1_ChilliFiesta_ExtraTimes_xuanzhong.csb")
