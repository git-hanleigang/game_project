...
--smy
--2018年4月18日
--FiestaDeMuertosWheelAction.lua


local FiestaDeMuertosWheelAction = class("FiestaDeMuertosWheelAction", util_require("base.BaseWheel"))

function FiestaDeMuertosWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function FiestaDeMuertosWheelAction:changeWheelRunData( Array)
...
function FiestaDeMuertosWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function FiestaDeMuertosWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function FiestaDeMuertosWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function FiestaDeMuertosWheelAction:recvData(index)
...
return FiestaDeMuertosWheelAction