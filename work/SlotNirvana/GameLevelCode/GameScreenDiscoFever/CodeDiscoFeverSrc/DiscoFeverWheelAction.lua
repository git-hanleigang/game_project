...
--smy
--2018年4月18日
--DiscoFeverWheelAction.lua


local DiscoFeverWheelAction = class("DiscoFeverWheelAction", util_require("base.BaseWheel"))

function DiscoFeverWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function DiscoFeverWheelAction:changeWheelRunData( Array)
...
function DiscoFeverWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function DiscoFeverWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function DiscoFeverWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function DiscoFeverWheelAction:recvData(index)
...
function DiscoFeverWheelAction:overWheel()
