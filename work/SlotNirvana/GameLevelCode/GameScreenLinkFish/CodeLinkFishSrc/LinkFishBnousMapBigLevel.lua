local LinkFishBnousMapBigLevel = class("LinkFishBnousMapBigLevel", util_require("base.BaseView"))
-- 构造函数
local DESCRIBE_LAYER_WIDTH = 180
function LinkFishBnousMapBigLevel:initUI(data)
    local resourceFilename = "Bonus_LinkFish_zhusunold_3.csb"
    ...
    local index = 1
    while true do
        local reel = self:findChild("LinkFish_" .. index )
        if reel ~= nil then
...
function LinkFishBnousMapBigLevel:updateExtraGame(info, pos, func)
    local showTittle = function ()
        ...
        local width = 0
        local addDistance = 8
        gLobalSoundManager:playSound("LinkFishSounds/sound_LinkFish_show_words.mp3")
...
function LinkFishBnousMapBigLevel:idle()
    self:runCsbAction("idleframe", true)
end

function LinkFishBnousMapBigLevel:click(func)
    gLobalSoundManager:playSound("LinkFishSounds/sound_LinkFish_big_level.mp3")
...
function LinkFishBnousMapBigLevel:levelReset()
...
function LinkFishBnousMapBigLevel:completed()
    -- self:runCsbAction("idleframe1", true)
end

function LinkFishBnousMapBigLevel:onEnter()
    
end

function LinkFishBnousMapBigLevel:onExit()
    
end

return LinkFishBnousMapBigLevel