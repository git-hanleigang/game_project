...
--smy
--2018年4月18日
--JackpotElvesWheelAction.lua


local JackpotElvesWheelAction = class("JackpotElvesWheelAction", util_require("base.BaseWheel"))

function JackpotElvesWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function JackpotElvesWheelAction:changeWheelRunData( Array)
...
function JackpotElvesWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function JackpotElvesWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function JackpotElvesWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function JackpotElvesWheelAction:recvData(index)
...
function JackpotElvesWheelAction:runWheel()
    self:changeWheel(self.m_curV)
    local time = self:getSpanTime()
    --减速前控制当前偏转角度在10度以内,避免停轮时偏差过大
    local offset = math.abs(self.m_currentDistance % 360) 
    if time >= self.m_runTime and self.m_isWheelData and offset <= 10 then
        self:prepareSlow()
    end
end

return JackpotElvesWheelAction