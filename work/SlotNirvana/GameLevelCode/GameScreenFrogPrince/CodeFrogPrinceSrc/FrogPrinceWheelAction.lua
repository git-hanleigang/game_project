...
--smy
--2018年4月18日
--FrogPrinceWheelAction.lua


local FrogPrinceWheelAction = class("FrogPrinceWheelAction", util_require("base.BaseWheel"))

function FrogPrinceWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
    ...
    self.m_backV = self.m_targetStep*1.5 --回弹时速度
    
end

function FrogPrinceWheelAction:changeWheelRunData( Array)
    ...
    -- self.m_backV = Array.m_backV
    self.m_func = Array.m_func
end

function FrogPrinceWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function FrogPrinceWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function FrogPrinceWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function FrogPrinceWheelAction:recvData(index,_type)
