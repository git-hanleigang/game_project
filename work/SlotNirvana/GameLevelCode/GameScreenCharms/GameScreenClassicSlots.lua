...
GameScreenClassicSlots.SYMBOL_SPECAIL_EMPTY = TAG_SYMBOL_TYPE.SYMBOL_INVALID_TYPE + 7
GameScreenClassicSlots.SYMBOL_SPECAIL_5X = TAG_SYMBOL_TYPE.SYMBOL_INVALID_TYPE + 8
GameScreenClassicSlots.SYMBOL_SPECAIL_3X = TAG_SYMBOL_TYPE.SYMBOL_INVALID_TYPE + 9
GameScreenClassicSlots.SYMBOL_SPECAIL_2X = TAG_SYMBOL_TYPE.SYMBOL_INVALID_TYPE + 10
-- 构造函数
...
function GameScreenClassicSlots:initMachine( )
    ...
    self:createCsbNode("Charms/GameScreenCharms_Classic.csb")
...
--     gLobalSoundManager:playSound("CharmsSounds/sound_classic_reelStop"..reelCol..".mp3")
--     -- end
...
-- function GameScreenClassicSlots:randomSlotNodesByReel( )
...
--- 处理spin 成功消息
--
function GameScreenClassicSlots:checkOperaSpinSuccess( param )
...
function GameScreenClassicSlots:onEnter()
    ...
    --     self:showDialog("ClassicStart", nil,function()
    --         performWithDelay(self, function()
    --             local random = math.random(1, 3)
    --             gLobalSoundManager:playSound("CharmsSounds/sound_classic_spin_"..random..".mp3")
    ...
    performWithDelay(self, function()
        local random = math.random(1, 3)
        self.m_spinSoundId = gLobalSoundManager:playSound("CharmsSounds/sound_classic_spin_"..random..".mp3")
        self:normalSpinBtnCall()
    end, 1)
end
...
function GameScreenClassicSlots:addObservers()
    ...
    gLobalNoticManager:addObserver(self,function(self,params)  -- 更新赢钱动画
        ...
        local soundName = "CharmsSounds/sound_classic_lastwin_".. soundIndex .. ".mp3"
...
function GameScreenClassicSlots:playEffectNotifyNextSpinCall( )
    if self.m_runSpinResultData.p_winAmount == 0 then
        ...
        --     local view = self:showDialog("ClassicOver", nil,function()
        --         performWithDelay(self, function()
        --             self.m_parent:resetProgress()
        ...
        --     node:setString(util_formatCoins(self.m_runSpinResultData.p_winAmount, 50))
        --     view:updateLabelSize({label=node,sx=0.8,sy=0.8},642)
        -- end, 2.5)
        local index = self.m_runSpinResultData.p_winLines[1].p_id

        local fileName = "Charms_Smallgame_1.csb"
        if index == 1 then
            fileName = "Charms_Smallgame_5.csb"
        elseif index == 2 or index == 3 then
            fileName = "Charms_Smallgame_4.csb"
        elseif index == 4 then
            fileName = "Charms_Smallgame_3.csb"
        elseif index == 5 or index == 6 then
            fileName = "Charms_Smallgame_2.csb"
        end
