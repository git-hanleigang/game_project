...
function CharmsRespinView:initUI(respinNodeName)
...
function CharmsRespinView:runNodeEnd(endNode)
      if not endNode or not endNode.p_symbolType then
            release_print("CharmsRespinView:respinNodeEndCallBack error!!!!")
            return
      end
      ...
      if self.m_machine:isFixSymbol(endNode.p_symbolType) then
            ...
            gLobalSoundManager:playSound("CharmsSounds/music_Charms_Respin_Bonus_Down.mp3")
...
function CharmsRespinView:oneReelDown()
      --gLobalSoundManager:playSound("CharmsSounds/music_Charms_reel_stop.mp3")
end
