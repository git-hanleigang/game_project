...
--xcyy
--2018年5月23日
--SpookySnacksShopView.lua
local SendDataManager = require "network.SendDataManager"
local SpookySnacksShopView = class("SpookySnacksShopView",util_require("Levels.BaseLevelDialog"))
...
function SpookySnacksShopView:initUI(params)
    self.m_machine = params.machine
    self.m_isTriggerSuper = false

    self.isExtraPick = false
    ...
    self:createCsbNode("SpookySnacks/ShopSpookySnacks.csb")
    ...
    self:findChild("Button"):setTag(BTN_TAG_CLOSE)

    for i=1,5 do
        --下面翻页 的小球
        self.m_pageNode[i] = util_createAnimation("SpookySnacks_shop_collect_diandian.csb")
        self:findChild("Node_page_"..i):addChild(self.m_pageNode[i])
        util_setCascadeOpacityEnabledRescursion(self:findChild("Node_page_"..i), true)
        util_setCascadeColorEnabledRescursion(self:findChild("Node_page_"..i), true)
    end

    for pageIndex = 1, 5 do
        local pageNode = util_createAnimation("SpookySnacks_shop_dian.csb")
        for index = 1,9 do
            local shopItem = util_createView("CodeSpookySnacksSrc.SpookySnacksShop.SpookySnacksShopItem",{parent = self,index = index})
    ...
    --message(小棋盘也在此文件中)
    self.shop_message = util_createAnimation("SpookySnacks_shop_message.csb")
    self:findChild("Node_message"):addChild(self.shop_message)

    --金币栏
    self.m_coinsBar = util_createAnimation("SpookySnacks_shop_qian.csb")
    ...
    --折扣标签
    self.m_discountBar = util_createAnimation("SpookySnacks_shop_zhekouquan.csb")
    self:findChild("Node_off"):addChild(self.m_discountBar)
    self.m_discountBar:setVisible(false)
    ...
    for tipsIndex = 1, 9 do
        --金币不足标签
        local tips = util_createAnimation("SpookySnacks_shop_shangpin_good.csb")
        self:addChild(tips)
...
function SpookySnacksShopView:addShopBg()
    local shopBg = util_createAnimation("SpookySnacks/GameScreenSpookySnacksBg.csb")
    shopBg:findChild("base"):setVisible(false)
    shopBg:findChild("fg"):setVisible(false)
    shopBg:findChild("respin"):setVisible(false)
    shopBg:findChild("shop"):setVisible(true)
    shopBg:findChild("superfg"):setVisible(false)
    self:findChild("bg"):addChild(shopBg)
end

function SpookySnacksShopView:onEnter()
...
-- function SpookySnacksShopView:showCoinNotEnough(_clickPos,_isHide,_isLocked)
...
function SpookySnacksShopView:cheakIsCanClick(_func)
    -- 正在移动 不能操作
...
--默认按钮监听回调
function SpookySnacksShopView:clickFunc(sender)
    ...
    self:cheakIsCanClick(function()
        if btnTag == BTN_TAG_LEFT then --左翻页
            ...
            self:hideShopItemTips(self.m_curPageIndex)
...
--默认按钮监听回调 滑动
function SpookySnacksShopView:clickEndFunc(sender)
...
--左右滑动切换背景和商店
function SpookySnacksShopView:onMoveClickCallBack(_sender)
    if self.m_isTriggerSuper then
        return
    end
 ...
 function SpookySnacksShopView:hideShopItemTips(pageindex)
    --刷新商店道具
    for index = 1,9 do
        local shopItem = self.m_pageCells[pageindex]:findChild("Node_shangpin"..index):getChildByTag(SHOP_ITEM_TAG)    
        if shopItem then
            shopItem:hideTipsForNode()
            shopItem:hideTipsForNode2()
        end
    end
 end
...
function SpookySnacksShopView:refreshView(_isSuperFreeBack, _direction, _isComeIn)
    self.m_clickPos = nil
    ...
    for index = 1,5 do
        ...
        self.shop_message:findChild("Node_"..index):setVisible(index == self.m_curPageIndex)
    end
    ...
    if _isSuperFreeBack then
        self.m_machine:delayCallBack(30/60,function()
            gLobalSoundManager:playSound(self.m_machine.m_publicConfig.SoundConfig.sound_SpookySnacks_shop_unLock)
        end)
    end
...
-- 翻页 --
function SpookySnacksShopView:moveNodeCells(_direction)
    if not _direction then
        return
    end

    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_SpookySnacks_shop_change_page)
...
function SpookySnacksShopView:updateCoins(_isComeIn)
    local shopConfig = self.m_machine.m_shopConfig
    local coins = shopConfig.coins
    self.m_coinsBar:findChild("m_lb_num1"):setString(util_formatCoins(coins,11))
    self:updateLabelSize({label=self.m_coinsBar:findChild("m_lb_num1"),sx=1,sy=1},150)
...
function SpookySnacksShopView:showView(_isSuperFreeBack)
    ...
    if not self.m_isZheKou then
        if self.m_discountBar:isVisible() then
            self.m_discountBar:runCsbAction("over",false,function ()
                self.m_discountBar:setVisible(false)
            end)
        else
            self.m_discountBar:setVisible(false)
        ...
        self.m_discountBar:setVisible(true)
        self.m_discountBar:runCsbAction("idle",false)
    end
    ...
    self.m_machine:resetMusicBg(false,"SpookySnacksSounds/music_SpookySnacks_shop.mp3")

    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_SpookySnacks_shop_open)

    self:runCsbAction("start",false, function()
        self.m_machine:changeUiForState(PublicConfig.uiState.shop)
...
function SpookySnacksShopView:hideView(_func)
    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_SpookySnacks_shop_hide)
    self.m_machine:changeUiForState(PublicConfig.uiState.base)
    self:runCsbAction("over",false, function()
        self.m_machine.m_bottomUI.m_changeLabJumpTime = nil
        self:setVisible(false)
        if _func then
...
function SpookySnacksShopView:clickItem(_clickPos)
...
function SpookySnacksShopView:sendData(_clickPos)
    if self.m_isWaiting then
        return
    end
...
function SpookySnacksShopView:featureResultCallFun(param)
...
function SpookySnacksShopView:recvBaseData(_featureData)
...
function SpookySnacksShopView:updateClickPosReward(_rewardData, _selfData, _featureData, _func)
    ...
    if #_rewardData > 2 and _rewardData[2] > 1 then
        ...
        if reward ~= "extraPick" then
            ...
            if not self.m_isTriggerSuper then
                gLobalNoticManager:postNotification(ViewEventType.NOTIFY_SPIN_BTN_STATUS, {SpinBtn_Type.BtnType_Spin, true})
            end
            shopItem:updateReward(reward, true, function()
                if _featureData.features[2] ~= SLOTO_FEATURE.FEATURE_FREESPIN then
...
function SpookySnacksShopView:flyCollectWinCois(_score,_startNode,_func) 
    local flyNode = util_createAnimation("SpookySnacks_shop_shangpin_jiangli.csb")

    flyNode:findChild("m_lb_coins"):setString(util_formatCoins(_score,3))
    self:updateLabelSize({label=flyNode:findChild("m_lb_coins"),sx=1,sy=1},125)
    flyNode:findChild("Node_2X"):setVisible(false)

    local startPos = util_convertToNodeSpace(_startNode:findChild("Node_number"),self)
    ...
    local seq = cc.Sequence:create({
        ...
        cc.CallFunc:create(function()
            gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_SpookySnacks_coinsFly_TotalWin)
        end),
        ...
        cc.CallFunc:create(function()
            ...
            self.m_machine:playBottomLight(_score, function()
                if type(_func) == "function" then
                    _func()
                end
            end)
            -- flyNode:findChild("Node_coin"):setVisible(false)
        end),
...
-- 成倍飞
function SpookySnacksShopView:collectChengBeiFly(_startpos, _endPos, _func)
    local shopItem = self.m_pageCells[self.m_curPageIndex]:findChild("Node_shangpin".._startpos):getChildByTag(SHOP_ITEM_TAG)
    shopItem.m_winCoinsNode:runCsbAction("idle2",false)
    ...
    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_SpookySnacks_chengbei_fly)
...
-- 显示当前的页
function SpookySnacksShopView:showCurPage(_pageIndex)
    for pageIndex = 1, 5 do
...
function SpookySnacksShopView:changeCoinNodeParent( )
    if not self.m_isZheKou then
