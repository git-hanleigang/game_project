...
--smy
--2018年4月18日
--SpookySnacksWheelAction.lua


local SpookySnacksWheelAction = class("SpookySnacksWheelAction", util_require("base.BaseWheel"))

function SpookySnacksWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function SpookySnacksWheelAction:changeWheelRunData( Array)
    ...
    self.m_slowQ = Array.m_slowQ --减速圈数
...
function SpookySnacksWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function SpookySnacksWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function SpookySnacksWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function SpookySnacksWheelAction:recvData(index)
    ...
    elseif self.m_dirType == 1 then
        ...
        self.m_randomDistance = self.m_targetStep*0.5
    end
...
return SpookySnacksWheelAction