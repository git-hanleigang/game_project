--[[
    ...
    以下接口需要在关卡内的直接重写:
        SpookySnacksSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        SpookySnacksSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        SpookySnacksSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        SpookySnacksSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        SpookySnacksSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local SpookySnacksSymbolExpect = class("SpookySnacksSymbolExpect")

function SpookySnacksSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function SpookySnacksSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function SpookySnacksSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function SpookySnacksSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function SpookySnacksSymbolExpect:isReelRunSymbolType(_symbolType)
...
function SpookySnacksSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function SpookySnacksSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function SpookySnacksSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function SpookySnacksSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
    if 0 ~= self.m_reelRunCol then
        --快滚时期待图标
        if self:isReelRunSymbolType(_slotNode.p_symbolType) then
            local iCol = _slotNode.p_cloumnIndex
            if iCol == self.m_reelRunCol - 1 then
                self:playExpectAnim(iCol, nil)
            elseif iCol > self.m_reelRunCol - 1 and iCol < self.m_machine.m_iReelColumnNum then
...
--播放期待动画 
function SpookySnacksSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function SpookySnacksSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function SpookySnacksSymbolExpect:stopExpectAnim()
...
--播放循环idle
function SpookySnacksSymbolExpect:playSymbolIdleAnim(_slotsNode)
    --一个关卡有多个循环idle图标时让动效统一命名时间线即可
    local loopIdleName = "idleframe2"
    if _slotsNode.p_symbolType == 94 then
        loopIdleName = "idle"
    end
    local bLoop        = true
    local curAnimName  = _slotsNode.m_currAnimName
...
return SpookySnacksSymbolExpect