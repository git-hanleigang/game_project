...
--smy
--2018年4月18日
--CoinCircusWheelAction.lua

local BaseWheel =  util_require("base.BaseWheel")
local CoinCircusWheelAction = class("CoinCircusWheelAction", BaseWheel)

function CoinCircusWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function CoinCircusWheelAction:changeWheelRunData( Array)
...
function CoinCircusWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end


function CoinCircusWheelAction:setWheelRotEndFunc( setRotEndFunc )
    self.m_setRotEndFunc = setRotEndFunc
end



--保存时间
function CoinCircusWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function CoinCircusWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function CoinCircusWheelAction:recvData(index)
...
function CoinCircusWheelAction:stopingWheel()

    BaseWheel.stopingWheel(self)

    if self.m_setRotEndFunc then
        self.m_setRotEndFunc()
    end

end

return CoinCircusWheelAction