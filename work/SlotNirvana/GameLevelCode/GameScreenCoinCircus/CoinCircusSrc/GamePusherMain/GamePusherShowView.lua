local GamePusherShowView = class("GamePusherShowView", util_require("base.BaseView"))
...
function GamePusherShowView:initUI(path)
    ...
    self:commonShow(
        ...
        function()
            ...
            if G_GetMgr(ACTIVITY_REF.CoinPusher):checkAutoDrop() then
                self:didClose()
            end
        end
    )
...
function GamePusherShowView:onEnter()
end
