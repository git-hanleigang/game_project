...
local ChameleonRichesLongRunControl = class("ChameleonRichesLongRunControl")

function ChameleonRichesLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function ChameleonRichesLongRunControl:getUsingReels()
    return self.usingReels
end

function ChameleonRichesLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function ChameleonRichesLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
    local reelDatas = self:getUsingReels()
    local lockNodes = self.m_machine.m_lockScatterSymbols
    local symbolNum = 0
    for k,v in pairs(lockNodes) do
    ...
    for index = 1, #_LegitimateCol do
        ...
        table.insert(info.LegitimatePos, {["iCol"] = iCol})
...
function ChameleonRichesLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function ChameleonRichesLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function ChameleonRichesLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function ChameleonRichesLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function ChameleonRichesLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function ChameleonRichesLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function ChameleonRichesLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function ChameleonRichesLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function ChameleonRichesLongRunControl:setLongRunLenAndStates()
    ...
    for iCol = startCol, self.m_machine.m_iReelColumnNum do
        ...
        if iCol == startCol then
            ...
            if startCol == 1 then
                ...
                self.m_machine.m_reelRunInfo[iCol - 1]:setNextReelLongRun(true)
...
function ChameleonRichesLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function ChameleonRichesLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function ChameleonRichesLongRunControl:clearCurLongRunData()
...
function ChameleonRichesLongRunControl:setFirstColBeginLongRun()
    if self.m_machine:getGameSpinStage() == QUICK_RUN then
        return
    end
    self.m_machine.m_isLongRun = true
    ...
    for i = 1, self.m_machine.m_iReelColumnNum do
        ...
        self.m_machine.m_baseReelNodes[i]:changeReelMoveSpeed(parentData.moveSpeed)
        
    end
    self.m_machine.m_symbolExpectCtr:playExpectAnim(1) 
end
...
return ChameleonRichesLongRunControl
