--[[
    ...
    以下接口需要在关卡内的直接重写:
        KoiBlissSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        KoiBlissSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        KoiBlissSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        KoiBlissSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        KoiBlissSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local KoiBlissSymbolExpect = class("KoiBlissSymbolExpect")

function KoiBlissSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function KoiBlissSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function KoiBlissSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function KoiBlissSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function KoiBlissSymbolExpect:isReelRunSymbolType(_symbolType)
...
function KoiBlissSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function KoiBlissSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function KoiBlissSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function KoiBlissSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function KoiBlissSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function KoiBlissSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function KoiBlissSymbolExpect:stopExpectAnim()
...
--播放循环idle
function KoiBlissSymbolExpect:playSymbolIdleAnim(_slotsNode)
    --一个关卡有多个循环idle图标时让动效统一命名时间线即可
    local loopIdleName = "idleframe1"
    -- if _slotsNode.p_symbolType == self.m_machine.SYMBOL_BONUS_1 then
    --     loopIdleName = "idleframe2"
    -- end
    local bLoop        = true
    local curAnimName  = _slotsNode.m_currAnimName
