...
--xcyy
--2018年5月23日
--KoiBlissJackpotWinView.lua
local PublicConfig = require "KoiBlissPublicConfig"
local KoiBlissJackpotWinView = class("KoiBlissJackpotWinView",util_require("Levels.BaseLevelDialog"))
...
function KoiBlissJackpotWinView:initUI(params)
    --jackpot类型统一转化为小写
    ...
    if self.m_jackpotIndex == JACKPOT_INDEX.grand then
        self:createCsbNode("KoiBliss/JackpotWinView_grand.csb") 
    else
        self:createCsbNode("KoiBliss/JackpotWinView.csb")
        for childName,node in pairs(self.m_csbOwner) do 
            if childName then
                for k,v in pairs(JACKPOT_INDEX) do
                    if string.find(childName,k) then
                        if k == self.m_viewType then
...
function KoiBlissJackpotWinView:showJackpotSoundForType()
    if self.m_jackpotIndex == JACKPOT_INDEX.grand then
        gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_KoiBliss_jackpot_grand)
    elseif self.m_jackpotIndex == JACKPOT_INDEX.major then
        gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_KoiBliss_jackpot_major)
    elseif self.m_jackpotIndex == JACKPOT_INDEX.minor then
        gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_KoiBliss_jackpot_minor)
    else
        gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_KoiBliss_jackpot_mini)
...
function KoiBlissJackpotWinView:showView(winCoin)
    self:showJackpotSoundForType()
...
function KoiBlissJackpotWinView:showOver()
    self.m_allowClick = false
    gLobalSoundManager:playSound(PublicConfig.SoundConfig.sound_KoiBliss_jackpot_hide)
...
function KoiBlissJackpotWinView:jumpCoins(params)
    ...
    local jumpSound = PublicConfig.SoundConfig.sound_KoiBliss_jump_coins --跳动音效
    local jumpSoundEnd = PublicConfig.SoundConfig.sound_KoiBliss_jump_coins_end --跳动结束音效
...
function KoiBlissJackpotWinView:clickFunc(sender)
...
function KoiBlissJackpotWinView:createGrandShare()
...
function KoiBlissJackpotWinView:jumpCoinsFinish()
...
function KoiBlissJackpotWinView:checkShareState()
    local bShare = false
    if nil ~= self.m_grandShare then
...
function KoiBlissJackpotWinView:jackpotViewOver(_fun)
    if nil ~= self.m_grandShare then
