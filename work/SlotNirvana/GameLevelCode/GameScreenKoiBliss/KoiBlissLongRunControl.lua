...
local KoiBlissLongRunControl = class("KoiBlissLongRunControl")

function KoiBlissLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function KoiBlissLongRunControl:getUsingReels()
    return self.usingReels
end

function KoiBlissLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function KoiBlissLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
    ...
    for index = 1, #_LegitimateCol do
        ...
        local isHave = false
        for iRow = 2, self.m_machine.m_iReelRowNum do
...
function KoiBlissLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function KoiBlissLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function KoiBlissLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function KoiBlissLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function KoiBlissLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function KoiBlissLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function KoiBlissLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function KoiBlissLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function KoiBlissLongRunControl:setLongRunLenAndStates()
...
function KoiBlissLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function KoiBlissLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function KoiBlissLongRunControl:clearCurLongRunData()
...
function KoiBlissLongRunControl:setFirstColBeginLongRun()
