--[[
    ...
    以下接口需要在关卡内的直接重写:
        RobinIsHoodSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        RobinIsHoodSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        RobinIsHoodSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        RobinIsHoodSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        RobinIsHoodSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local RobinIsHoodSymbolExpect = class("RobinIsHoodSymbolExpect")

function RobinIsHoodSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function RobinIsHoodSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function RobinIsHoodSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function RobinIsHoodSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function RobinIsHoodSymbolExpect:isReelRunSymbolType(_symbolType)
...
function RobinIsHoodSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function RobinIsHoodSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function RobinIsHoodSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function RobinIsHoodSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function RobinIsHoodSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function RobinIsHoodSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function RobinIsHoodSymbolExpect:stopExpectAnim()
...
--播放循环idle
function RobinIsHoodSymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return RobinIsHoodSymbolExpect