...
--xcyy
--2018年5月23日
--RobinIsHoodJackpotWinView.lua
local PublicConfig = require "RobinIsHoodPublicConfig"
local RobinIsHoodJackpotWinView = class("RobinIsHoodJackpotWinView",util_require("Levels.BaseLevelDialog"))
...
function RobinIsHoodJackpotWinView:initUI(params)
    --jackpot类型统一转化为小写
    ...
    gLobalSoundManager:playSound(PublicConfig.SoundConfig["sound_RobinIsHood_show_jackpot_view_"..viewType])

    if viewType == "grand" then
        self:createCsbNode("RobinIsHood/JackpotWinView_grand.csb")
    else
        self:createCsbNode("RobinIsHood/JackpotWinView.csb")
    ...
    self.m_light = util_createAnimation("RobinIsHood_tanban_guang.csb")
    self:findChild("Node_guang2"):addChild(self.m_light)
    self.m_light:runCsbAction("idle2",true)
    
    util_setCascadeOpacityEnabledRescursion(self:findChild("Node_guang2"),true)
...
function RobinIsHoodJackpotWinView:initSpineUI()
    self.m_spine = util_spineCreate("RobinIsHood_juese",true,true)
    self:findChild("spine"):addChild(self.m_spine)
    util_spinePlay(self.m_spine,"jackpot_idle",true)
...
function RobinIsHoodJackpotWinView:showView(winCoin)
...
function RobinIsHoodJackpotWinView:showOver()
    self.m_allowClick = false
    gLobalSoundManager:playSound(PublicConfig.SoundConfig["sound_RobinIsHood_hide_jp_view"])
...
function RobinIsHoodJackpotWinView:jumpCoins(params)
    ...
    local jumpSound = PublicConfig.SoundConfig.sound_RobinIsHood_jackpot_jump_coins --跳动音效
    local jumpSoundEnd = PublicConfig.SoundConfig.sound_RobinIsHood_jackpot_jump_coins_end --跳动结束音效
...
function RobinIsHoodJackpotWinView:clickFunc(sender)
    if not self.m_allowClick then
        return
    end

    --点击音效
    if PublicConfig.SoundConfig.sound_RobinIsHood_click then
...
function RobinIsHoodJackpotWinView:createGrandShare()
...
function RobinIsHoodJackpotWinView:jumpCoinsFinish()
...
function RobinIsHoodJackpotWinView:checkShareState()
    local bShare = false
    if nil ~= self.m_grandShare then
...
function RobinIsHoodJackpotWinView:jackpotViewOver(_fun)
    if nil ~= self.m_grandShare then
...
return RobinIsHoodJackpotWinView