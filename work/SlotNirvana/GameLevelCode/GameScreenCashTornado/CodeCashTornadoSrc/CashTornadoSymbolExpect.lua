--[[
    ...
    以下接口需要在关卡内的直接重写:
        CashTornadoSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        CashTornadoSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        CashTornadoSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        CashTornadoSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        CashTornadoSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local CashTornadoSymbolExpect = class("CashTornadoSymbolExpect")

function CashTornadoSymbolExpect:initData_(_machine)
    ...
    self.m_idleSymbolTypeList     = {                     --循环idle类型的图标列表
        ...
        self.m_machine.SYMBOL_SCORE_BONUS3,
        self.m_machine.SYMBOL_SCORE_BONUS4,
        self.m_machine.SYMBOL_SCORE_BONUS5,
    }
end

-- 接口在关卡内直接重写
--[[
function CashTornadoSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function CashTornadoSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function CashTornadoSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function CashTornadoSymbolExpect:isReelRunSymbolType(_symbolType)
...
function CashTornadoSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function CashTornadoSymbolExpect:getNextReelLongRunState(_iCol)
    ...
    if _iCol == 6 then
        reelRunData = self.m_machine.m_reelRunInfo[5]
    end
...
--循环idle图标检测
function CashTornadoSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function CashTornadoSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function CashTornadoSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function CashTornadoSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function CashTornadoSymbolExpect:stopExpectAnim()
...
--播放循环idle
function CashTornadoSymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return CashTornadoSymbolExpect