...
local CashTornadoLongRunControl = class("CashTornadoLongRunControl")

function CashTornadoLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function CashTornadoLongRunControl:getUsingReels()
    return self.usingReels
end

function CashTornadoLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function CashTornadoLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
    local reelDatas = self:getUsingReels()
    local symbolNum = 0
    local info = {}
    info.startCol = 1
    info.endCol = self.m_machine.m_iReelColumnNum
    info.LegitimatePos = {}
...
function CashTornadoLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
    ...
    if not info.longRun then
        for i=#info.LegitimatePos,1,-1 do
            ...
            if posInfo.iCol == self.m_machine.m_iReelColumnNum then
                table.remove( info.LegitimatePos, i )
            elseif posInfo.iCol == self.m_machine.m_iReelColumnNum - 1 then
                table.remove( info.LegitimatePos, i )
            end
        end
    end
...
function CashTornadoLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CashTornadoLongRunControl:getLongRun15ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
    local legitimateCol = {1,5}
    local legitimateNum = _legitimateNum or 2
    local info = self:getLongRunInfoFromReel(_symbolTypes, legitimateCol, legitimateNum)
    -- if not info.longRun then
    --     for i=#info.LegitimatePos,1,-1 do
    ...
    --     info.startCol = 4
    -- end
    info.unlegitimateCol = {2,3,4} 
    return info
end

function CashTornadoLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function CashTornadoLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function CashTornadoLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function CashTornadoLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function CashTornadoLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
    ...
    elseif _longRunId == self.Enum_LongRunId["15"] then
        infos = self:getLongRun15ColInfos(_symbolTypes, _legitimateNum)
...
function CashTornadoLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function CashTornadoLongRunControl:setLongRunLenAndStates()
...
function CashTornadoLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function CashTornadoLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function CashTornadoLongRunControl:clearCurLongRunData()
...
function CashTornadoLongRunControl:setFirstColBeginLongRun()
...
return CashTornadoLongRunControl
