...
--island
--2018年4月12日
--BlazingMotorsWheelNode.lua
--
-- jackpot top bar
local SpeicalReel = require "Levels.SpeicalReel"
local BlazingMotorsWheelNode = class("BlazingMotorsWheelNode",SpeicalReel)

BlazingMotorsWheelNode.m_runState = nil
BlazingMotorsWheelNode.m_runSpeed = nil
BlazingMotorsWheelNode.m_endRunData = nil
BlazingMotorsWheelNode.m_allRunSymbols = nil
...
-- local SLOW_SPEED = 30
-- local SLOW_TIME = 1
BlazingMotorsWheelNode.m_runActions = nil
BlazingMotorsWheelNode.m_runNowAction = nil
...
BlazingMotorsWheelNode.m_endCallBackFun = nil

BlazingMotorsWheelNode.m_reelHeight = nil

--状态切换
BlazingMotorsWheelNode.m_timeCount = 0
BlazingMotorsWheelNode.m_countDownTime = BEGIN_ACC_DELAY_TIME

BlazingMotorsWheelNode.m_randomDataFunc = nil

BlazingMotorsWheelNode.distance = 0

function BlazingMotorsWheelNode:initUI()
...
function BlazingMotorsWheelNode:initAction()
...
function BlazingMotorsWheelNode:initFirstSymbolBySymbols(initDataList, reelHeight)
...
function BlazingMotorsWheelNode:getDisToReelLowBoundary(node)
...
function BlazingMotorsWheelNode:getNextRunData()
...
function BlazingMotorsWheelNode:createNextNode()
...
function BlazingMotorsWheelNode:getNextMoveActions()
...
--设置滚动序列
function BlazingMotorsWheelNode:getMoveActions()
      local runActions = {}
...
--重写每帧走的距离
function BlazingMotorsWheelNode:setDtMoveDis(dt)
...
function BlazingMotorsWheelNode:getIsTimeDown(actionTime)
...
function BlazingMotorsWheelNode:initRunDate(runData, getRunDatafunc)
...
function BlazingMotorsWheelNode:setAllRunSymbols( allSymbols )
      self.m_allRunSymbols = allSymbols
end

function BlazingMotorsWheelNode:setEndDate(endData)
      self.m_endRunData = endData
end

function BlazingMotorsWheelNode:beginMove()

      if self:getReelStatus() ~= REEL_STATUS.IDLE then
          return 
      end

      self.wheelRunSoundId = gLobalSoundManager:playSound("BlazingMotorsSounds/music_BlazingMotors_Wheel_run.mp3") 
...
function BlazingMotorsWheelNode:changeRunState(dt)
...
function BlazingMotorsWheelNode:getResAction()
      local timeDown = 0
...
function BlazingMotorsWheelNode:runResAction()
...
function BlazingMotorsWheelNode:playRunEndAnima()
      for i=1,# self.m_symbolNodeList do
           local node =  self.m_symbolNodeList[i]
      --      node:setVisible(false)
            
      
           if self.wheelRunSoundId then
                  gLobalSoundManager:stopAudio(self.wheelRunSoundId)
                  self.wheelRunSoundId = nil
           end  

           gLobalSoundManager:playSound("BlazingMotorsSounds/music_BlazingMotors_Wheel_run_stop.mp3") 
...
function BlazingMotorsWheelNode:setEndCallBackFun(func)
      self.m_endCallBackFun = func
end



return BlazingMotorsWheelNode