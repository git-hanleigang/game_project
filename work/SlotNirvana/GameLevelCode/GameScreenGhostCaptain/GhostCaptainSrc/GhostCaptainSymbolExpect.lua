--[[
    ...
    关卡创建:
        self.m_symbolExpectCtr = util_createView("CodeGhostCaptainSrc.GhostCaptainSymbolExpect", self)
    以下接口需要在关卡内的直接重写:
        GhostCaptainSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        GhostCaptainSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        GhostCaptainSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        GhostCaptainSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        GhostCaptainSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local GhostCaptainSymbolExpect = class("GhostCaptainSymbolExpect")

function GhostCaptainSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function GhostCaptainSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function GhostCaptainSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function GhostCaptainSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function GhostCaptainSymbolExpect:isReelRunSymbolType(_symbolType)
...
function GhostCaptainSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function GhostCaptainSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function GhostCaptainSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function GhostCaptainSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function GhostCaptainSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function GhostCaptainSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function GhostCaptainSymbolExpect:stopExpectAnim(_isPlay)
    ...
    for iCol=1,maxCol do
        for iRow=1,maxRow do
            local slotsNode = self.m_machine:getFixSymbol(iCol, iRow, SYMBOL_NODE_TAG)
            if slotsNode and self:isReelRunSymbolType(slotsNode.p_symbolType) then 
                self:playSymbolIdleAnim(slotsNode, _isPlay)
            end
        end
    end
end
--播放循环idle
function GhostCaptainSymbolExpect:playSymbolIdleAnim(_slotsNode, _isPlay)
    ...
    if _isPlay and self.m_machine.m_runSpinResultData.p_features and #self.m_machine.m_runSpinResultData.p_features > 0 and 
        self.m_machine.m_runSpinResultData.p_features[2] == 1 then 
            loopIdleName = "idleframe"
    end
    --spine混合
    --[[
        local ccbNode = _slotsNode:checkLoadCCbNode()
...
return GhostCaptainSymbolExpect