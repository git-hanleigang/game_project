...
--smy
--2018年4月18日
--OZBonus_WheelAction.lua


local OZBonus_WheelAction = class("OZBonus_WheelAction", util_require("base.BaseWheel"))

function OZBonus_WheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function OZBonus_WheelAction:changeWheelRunData( Array)
...
function OZBonus_WheelAction:updateEndCallFunc( func )
    self.m_func = func
end



function OZBonus_WheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function OZBonus_WheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function OZBonus_WheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function OZBonus_WheelAction:recvData(index)
