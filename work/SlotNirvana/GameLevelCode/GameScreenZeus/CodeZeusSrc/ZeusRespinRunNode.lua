...
--island
--2018年4月12日
--ZeusRespinRunNode.lua
--
-- jackpot top bar
local SpeicalReel = require "Levels.SpeicalReel"
local ZeusRespinRunNode = class("ZeusRespinRunNode",SpeicalReel)
...
ZeusRespinRunNode.m_runState = nil
ZeusRespinRunNode.m_runSpeed = nil
ZeusRespinRunNode.m_endRunData = nil
ZeusRespinRunNode.m_allRunSymbols = nil

--状态
ZeusRespinRunNode.m_runActions = nil
ZeusRespinRunNode.m_runNowAction = nil
...
ZeusRespinRunNode.m_endCallBackFun = nil

ZeusRespinRunNode.m_reelHeight = nil

--状态切换
ZeusRespinRunNode.m_timeCount = 0
ZeusRespinRunNode.m_countDownTime = BEGIN_ACC_DELAY_TIME

ZeusRespinRunNode.m_randomDataFunc = nil

ZeusRespinRunNode.distance = 0

function ZeusRespinRunNode:initUI()
...
function ZeusRespinRunNode:init(wildth ,height, getSlotNodeFunc, pushSlotNodeFunc)
...
function ZeusRespinRunNode:initAction()
...
function ZeusRespinRunNode:initFirstSymbolBySymbols(initDataList, reelHeight)
...
function ZeusRespinRunNode:getDisToReelLowBoundary(node)
...
function ZeusRespinRunNode:getNextRunData()
...
function ZeusRespinRunNode:createNextNode()
...
function ZeusRespinRunNode:getNextMoveActions()
...
--设置滚动序列
function ZeusRespinRunNode:getMoveActions()
      local runActions = {}
...
--重写每帧走的距离
function ZeusRespinRunNode:setDtMoveDis(dt)
...
function ZeusRespinRunNode:getIsTimeDown(actionTime)
...
function ZeusRespinRunNode:initRunDate(runData, getRunDatafunc)
...
function ZeusRespinRunNode:setAllRunSymbols( allSymbols )
      self.m_allRunSymbols = allSymbols
end

function ZeusRespinRunNode:setEndDate(endData)
      self.m_endRunData = endData
end

function ZeusRespinRunNode:beginMove()
...
function ZeusRespinRunNode:changeRunState(dt)
...
-- function ZeusRespinRunNode:createCsbNode(filePath)
...
function ZeusRespinRunNode:getResAction()
      local timeDown = 0
...
function ZeusRespinRunNode:runResAction()
...
function ZeusRespinRunNode:playRunEndAnima()
...
function ZeusRespinRunNode:playRunEndAnimaIde()
      for i=1,# self.m_symbolNodeList do
            local node =  self.m_symbolNodeList[i]
...
function ZeusRespinRunNode:setEndCallBackFun(func)
      self.m_endCallBackFun = func
end



return ZeusRespinRunNode