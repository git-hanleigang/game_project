...
--xcyy
--2018年5月23日
--MuchoChilliRespinNode.lua

local MuchoChilliRespinNode = class("MuchoChilliRespinNode",util_require("Levels.RespinNode"))

local NODE_TAG = 10
local MOVE_SPEED = 2000     --滚动速度 像素/每秒
local RES_DIS = 20
MuchoChilliRespinNode.SYMBOL_LOCAL_BONUS = 110
MuchoChilliRespinNode.SYMBOL_LOCAL_SPECIAL_BONUS = 111
MuchoChilliRespinNode.SYMBOL_EMPTY = 100
MuchoChilliRespinNode.SYMBOL_BONUS = 94
MuchoChilliRespinNode.SYMBOL_SPECIAL_BONUS = 95
MuchoChilliRespinNode.m_respinLastSymbolType = 100

-- 构造函数
function MuchoChilliRespinNode:ctor()
    MuchoChilliRespinNode.super.ctor(self)
    self.m_isQuick = false
    self.m_isLastRespin = false
end

--裁切遮罩透明度
function MuchoChilliRespinNode:initClipOpacity(opacity)

end

--获取随机信号
function MuchoChilliRespinNode:randomRuningSymbolType()
    local lastSymbolType = self.m_respinLastSymbolType

    local nodeType = nil
    if lastSymbolType then
        if lastSymbolType == self.SYMBOL_EMPTY then--上一个是空
            if math.random(1, 100) <= 50 then
                nodeType = self.SYMBOL_LOCAL_BONUS
            else
    ...
    if self.m_machine.m_isMiniMachine then
        if self.m_machine.m_parent:getRespinType() == nil then
            if nodeType == self.SYMBOL_LOCAL_BONUS then
                nodeType = self.SYMBOL_LOCAL_SPECIAL_BONUS
            end
        end
    else
        if self.m_machine:getRespinType() == nil then
            if nodeType == self.SYMBOL_LOCAL_BONUS then
                nodeType = self.SYMBOL_LOCAL_SPECIAL_BONUS
            end
        end
...
--创建下一个节点
function MuchoChilliRespinNode:baseCreateNextNode()
...
---------------------------------快滚相关-------------------------------------------
--设置滚动速度
function MuchoChilliRespinNode:changeRunSpeed(isQuick, isLastRespin)
    if isQuick then
...
--设置回弹距离
function MuchoChilliRespinNode:changeResDis(isQuick, isLastRespin)
    if isQuick then
...
--获取回弹动作序列
function MuchoChilliRespinNode:getBaseResAction(startPos)
    local timeDown = 0
...
--执行回弹动作
function MuchoChilliRespinNode:runBaseResAction()
