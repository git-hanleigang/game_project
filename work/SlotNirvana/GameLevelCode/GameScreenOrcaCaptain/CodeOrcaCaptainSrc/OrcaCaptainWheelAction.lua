...
--smy
--2018年4月18日
--OrcaCaptainWheelAction.lua

local OrcaCaptainPublicConfig = require "OrcaCaptainPublicConfig"
local OrcaCaptainWheelAction = class("OrcaCaptainWheelAction", util_require("base.BaseWheel"))

function OrcaCaptainWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function OrcaCaptainWheelAction:changeWheelRunData( Array)
    ...
    self.m_currentDistance = Array.m_currentDistance
end

function OrcaCaptainWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function OrcaCaptainWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function OrcaCaptainWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function OrcaCaptainWheelAction:recvData(index)
