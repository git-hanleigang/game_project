--[[
    ...
    以下接口需要在关卡内的直接重写:
        OrcaCaptainSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        OrcaCaptainSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        OrcaCaptainSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        OrcaCaptainSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        OrcaCaptainSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local OrcaCaptainSymbolExpect = class("OrcaCaptainSymbolExpect")

function OrcaCaptainSymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function OrcaCaptainSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function OrcaCaptainSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function OrcaCaptainSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function OrcaCaptainSymbolExpect:isReelRunSymbolType(_symbolType)
...
function OrcaCaptainSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function OrcaCaptainSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function OrcaCaptainSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function OrcaCaptainSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function OrcaCaptainSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function OrcaCaptainSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function OrcaCaptainSymbolExpect:stopExpectAnim()
    ...
    for iCol=1,maxCol do
        for iRow=1,maxRow do
            local slotsNode = self.m_machine:getFixSymbol(iCol, iRow, SYMBOL_NODE_TAG)
            if slotsNode and self:isReelRunSymbolType(slotsNode.p_symbolType) then 
                if slotsNode.m_currAnimName == "idleframe3" then
                    self:playSymbolIdleAnim(slotsNode)
                end
            end
        end
...
--播放循环idle
function OrcaCaptainSymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return OrcaCaptainSymbolExpect