...
local OrcaCaptainLongRunControl = class("OrcaCaptainLongRunControl")

function OrcaCaptainLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function OrcaCaptainLongRunControl:getUsingReels()
    return self.usingReels
end

function OrcaCaptainLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function OrcaCaptainLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function OrcaCaptainLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function OrcaCaptainLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function OrcaCaptainLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function OrcaCaptainLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function OrcaCaptainLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function OrcaCaptainLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function OrcaCaptainLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function OrcaCaptainLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function OrcaCaptainLongRunControl:setLongRunLenAndStates()
...
function OrcaCaptainLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function OrcaCaptainLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function OrcaCaptainLongRunControl:clearCurLongRunData()
...
function OrcaCaptainLongRunControl:setFirstColBeginLongRun()
...
return OrcaCaptainLongRunControl
