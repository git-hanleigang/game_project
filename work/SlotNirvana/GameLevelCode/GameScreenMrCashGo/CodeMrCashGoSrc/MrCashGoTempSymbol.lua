local MrCashGoTempSymbol = class("MrCashGoTempSymbol", cc.Node)


-- 构造函数
function MrCashGoTempSymbol:ctor()
    self.m_symbolType = -1
    self.m_ccbName    = ""
    self.m_animNode   = nil
end

function MrCashGoTempSymbol:initData_(_machine)
    self.m_machine   = _machine
end

function MrCashGoTempSymbol:changeSymbolCcb(_symbolType)
...
function MrCashGoTempSymbol:createAnimNode()
...
function MrCashGoTempSymbol:upDateAnimNode()
...
function MrCashGoTempSymbol:runAnim(animName,loop,func)
    self:createAnimNode()
