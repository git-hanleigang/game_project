...
local OwlsomeWizardLongRunControl = class("OwlsomeWizardLongRunControl")

function OwlsomeWizardLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function OwlsomeWizardLongRunControl:getUsingReels()
    return self.usingReels
end

function OwlsomeWizardLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function OwlsomeWizardLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function OwlsomeWizardLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function OwlsomeWizardLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function OwlsomeWizardLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function OwlsomeWizardLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function OwlsomeWizardLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function OwlsomeWizardLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function OwlsomeWizardLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function OwlsomeWizardLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function OwlsomeWizardLongRunControl:setLongRunLenAndStates()
...
function OwlsomeWizardLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function OwlsomeWizardLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function OwlsomeWizardLongRunControl:clearCurLongRunData()
...
function OwlsomeWizardLongRunControl:setFirstColBeginLongRun()
...
return OwlsomeWizardLongRunControl
