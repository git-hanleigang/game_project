-- 临时图标效果
local CherryBountyTempSymbol = class("CherryBountyTempSymbol", cc.Node)

-- 构造函数
function CherryBountyTempSymbol:ctor()
...
function CherryBountyTempSymbol:initData_(_initData)
    --初始化一些属性
    --[[
        _initData = {
            iCol       = 1,
...
function CherryBountyTempSymbol:removeTempSlotsNode()
...
function CherryBountyTempSymbol:changeSymbolCcb(_symbolType)
...
function CherryBountyTempSymbol:createAnimNode()
...
function CherryBountyTempSymbol:upDateAnimNode()
...
function CherryBountyTempSymbol:checkLoadCCbNode()
    self:createAnimNode()
    return self.m_animNode
end
function CherryBountyTempSymbol:getCCBNode()
    return self.m_animNode
end
--[[
    时间线相关
...
--执行混合动作
function CherryBountyTempSymbol:runMixAni(aniName,loop,func,curAniName,time)
...
function CherryBountyTempSymbol:runAnim(animName,loop,func)
    self:createAnimNode()
    if not self.m_animNode then
        if func ~= nil then
            func()
        end
        local sMsg = strnig.format("[CherryBountyTempSymbol:runAnim] m_symbolType=(%d)", self.m_symbolType)
        error(sMsg)
        return
    end
...
function CherryBountyTempSymbol:registerAniamCallBackFun(_fun)
    self.m_fnRunAnimCallBack = _fun
end
--[[
    获取动画节点上面的子节点
]]
function CherryBountyTempSymbol:getCcbProperty(propName)
...
function CherryBountyTempSymbol:getAniamDurationByName(_animName)
    local mainClass = self.m_machine
    local spineSymbolData = mainClass.m_configData:getSpineSymbol(self.m_symbolType)
