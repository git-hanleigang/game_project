...
local WinningFishRespinNode = class("WinningFishRespinNode",RespinNode)

local MOVE_SPEED = 1500     --滚动速度 像素/每秒
local RES_DIS = 20

--裁切遮罩透明度
function WinningFishRespinNode:initClipOpacity(opacity)
      if opacity and opacity>0 then
            ...
            self.m_colorNode = util_createAnimation("Socre_WinningFish_Linkdiban.csb")--util_createColorMask(RESPIN_COLOR_TYPE.LAYERCOLOR,pos,clipSize,opacity,spPath)
...
--设置滚动速度
function WinningFishRespinNode:changeRunSpeed(isQuick)
      if isQuick then
...
--设置回弹距离
function WinningFishRespinNode:changeResDis(isQuick)
      if isQuick then
...
--执行回弹动作
function WinningFishRespinNode:runBaseResAction()
...
--获取回弹动作序列
function WinningFishRespinNode:getBaseResAction(startPos)
      local timeDown = 0
