...
--smy
--2018年4月18日
--DragonsWheelAction.lua


local DragonsWheelAction = class("DragonsWheelAction", util_require("base.BaseWheel"))

function DragonsWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function DragonsWheelAction:changeWheelRunData( Array)
...
function DragonsWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function DragonsWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function DragonsWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function DragonsWheelAction:recvData(index)
