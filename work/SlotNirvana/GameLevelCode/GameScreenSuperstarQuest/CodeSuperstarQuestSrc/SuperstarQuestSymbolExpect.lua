...
local SuperstarQuestSymbolExpect = class("SuperstarQuestSymbolExpect")

function SuperstarQuestSymbolExpect:initData_(params)
...
function SuperstarQuestSymbolExpect:MachineOneReelDownCall(colIndex,specialList)
    if colIndex == 1 then
        self.m_bulingEndSymbols = {}
    end
...
function SuperstarQuestSymbolExpect:checkIsExpectSymbol(symbolType)
...
function SuperstarQuestSymbolExpect:MachineSymbolBulingEndCall(symbolNode)
...
function SuperstarQuestSymbolExpect:playSymbolExpectAnim(_slotsNode)
    if tolua.isnull(_slotsNode) then
        return
    end
    ...
    if animName == "" or animName == curAnimName then
        return
    end
...
function SuperstarQuestSymbolExpect:getAniName(symbolType,isIdle)
    for k,data in pairs(self.m_symbolList) do
        local list = data.symbolTypeList
        for index,triggerType in pairs(list) do
            if triggerType == symbolType then
...
--播放循环idle
function SuperstarQuestSymbolExpect:playSymbolIdleAnim(_slotsNode)
    --没播完落地的等播完落地再播idle
    local symbolTag = self.m_machine:getNodeTag(_slotsNode.p_cloumnIndex, _slotsNode.p_rowIndex, SYMBOL_NODE_TAG)
...
return SuperstarQuestSymbolExpect