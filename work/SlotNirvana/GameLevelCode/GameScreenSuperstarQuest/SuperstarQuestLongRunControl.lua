...
local SuperstarQuestLongRunControl = class("SuperstarQuestLongRunControl")
...
function SuperstarQuestLongRunControl:initData_(params)
...
function SuperstarQuestLongRunControl:checkTriggerLongRun(lockNodesInfo)
    self.longRunStatus = {}
    --预告中奖时不触发快滚
...
function SuperstarQuestLongRunControl:checkLongRunBySymbolType(triggerCfg,lockNodesInfo)
...
function SuperstarQuestLongRunControl:setLongRunInfo(colIndex)
    if colIndex < 4 then
        return
    end
    --已经是快滚状态的列不需要再次设置
...
function SuperstarQuestLongRunControl:setFirstColBeginLongRun()
...
return SuperstarQuestLongRunControl
