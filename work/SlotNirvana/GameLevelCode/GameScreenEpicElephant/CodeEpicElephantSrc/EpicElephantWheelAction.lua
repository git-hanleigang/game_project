...
--smy
--2018年4月18日
--EpicElephantWheelAction.lua


local EpicElephantWheelAction = class("EpicElephantWheelAction", util_require("base.BaseWheel"))

function EpicElephantWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function EpicElephantWheelAction:changeWheelRunData( Array)
...
function EpicElephantWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function EpicElephantWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function EpicElephantWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function EpicElephantWheelAction:recvData(index)
...
return EpicElephantWheelAction