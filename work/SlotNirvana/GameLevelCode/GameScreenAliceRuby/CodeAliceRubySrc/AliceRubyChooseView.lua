...
--xcyy
--2018年5月23日
--AliceRubyChooseView.lua

local SpinFeatureData = require "data.slotsdata.SpinFeatureData"
local SendDataManager = require "network.SendDataManager"
local BaseGame = util_require("base.BaseGame")
local AliceRubyChooseView = class("AliceRubyChooseView", BaseGame)

AliceRubyChooseView.m_ClickIndex = 1
AliceRubyChooseView.m_spinDataResult = {}

function AliceRubyChooseView:initUI(machine)
    local isAutoScale = true
    if CC_RESOLUTION_RATIO == 3 then
        isAutoScale = false
    end

    self:createCsbNode("AliceRuby/GameChose.csb", isAutoScale)
...
function AliceRubyChooseView:updateChooseData()
...
function AliceRubyChooseView:updateLable()
    for i = 1, 3 do
...
-- function AliceRubyChooseView:onEnter()
...
function AliceRubyChooseView:onExit()
    self:clearBaseData()
    gLobalNoticManager:removeAllObservers(self)
end

function AliceRubyChooseView:checkAllBtnClickStates()
...
--默认按钮监听回调
function AliceRubyChooseView:clickFunc(sender)
...
--数据接收
function AliceRubyChooseView:recvBaseData(featureData)
    self.m_isStart_Over_Action = true
...
--数据发送
function AliceRubyChooseView:sendData(pos)
...
function AliceRubyChooseView:featureResultCallFun(param)
...
--弹出结算奖励
function AliceRubyChooseView:showReward()
    if self.m_bonusEndCall then
        self.m_bonusEndCall()
    end
end

function AliceRubyChooseView:setEndCall(func)
    self.m_bonusEndCall = func
end

function AliceRubyChooseView:closeUi(func)
    self:runCsbAction(
        "over",
        false,
        function()
