...
--xcyy
--2018年5月23日
--CoinConiferColofulJackPotBar.lua
local PublicConfig = require "CoinConiferPublicConfig"
local CoinConiferColofulJackPotBar = class("CoinConiferColofulJackPotBar",util_require("Levels.BaseLevelDialog"))
...
function CoinConiferColofulJackPotBar:initUI()
    self:createCsbNode("CoinConifer_jackpot_jackpotbar.csb")
    self:runIdleAni()
...
function CoinConiferColofulJackPotBar:onEnter()

    CoinConiferColofulJackPotBar.super.onEnter(self)
...
function CoinConiferColofulJackPotBar:onExit()
    CoinConiferColofulJackPotBar.super.onExit(self)
end

function CoinConiferColofulJackPotBar:initMachine(machine)
    self.m_machine = machine
end

function CoinConiferColofulJackPotBar:adddianjiEffect()
    self.dianjiList = {}
    for jackpotType,jackpotIndex in pairs(JACKPOT_INDEX) do
...
--重置某一个jackpot
function CoinConiferColofulJackPotBar:resetViewForJackpot(jackpotType)
...
function CoinConiferColofulJackPotBar:resetView(isDisconnection)
...
function CoinConiferColofulJackPotBar:changejiManInfo()
    local freespinExtra = self.m_machine.m_runSpinResultData.p_fsExtraData or {}
    local jackpotnum = clone(freespinExtra.jackpotnum or {})
    if table_length(jackpotnum) > 0 then
...
function CoinConiferColofulJackPotBar:collectFeedBackAni(jackpotType,pointItem)
    if pointItem then
...
--待触发动效
function CoinConiferColofulJackPotBar:nextTriggerAni(jackpotType)
    local process = self.m_collectProcess[jackpotType]
    local nextNum = 4
    if jackpotType == "mini" then
        nextNum = 3
...
function CoinConiferColofulJackPotBar:resetNextTriggerItemAct()
    for jackpotType,itemList in pairs(self.m_collectItems) do
        self.m_collectProcess[jackpotType] = 0
        for index,item in ipairs(itemList) do
            if not tolua.isnull(item) and item.m_runActName == "actionframe2" then
                item:runCsbAction("idle1")
                item.m_runActName = "idle1"
            end
        end
...
function CoinConiferColofulJackPotBar:getFeedBackPoint(jackpotType,index)
    local itemList = self.m_collectItems[jackpotType]
...
function CoinConiferColofulJackPotBar:getProcessByType(jackpotType)
...
function CoinConiferColofulJackPotBar:getProcessByType2(jackpotType)
    return self.m_collectProcess[jackpotType]
end
--[[
    显示中奖光效
]]
function CoinConiferColofulJackPotBar:showHitLight(jackpotType)
...
function CoinConiferColofulJackPotBar:changeShowItem(item,jackpotType)
...
function CoinConiferColofulJackPotBar:runIdleAni()
    self:runCsbAction("idle",true)
end

-- 更新jackpot 数值信息
--
function CoinConiferColofulJackPotBar:updateJackpotInfo()
...
function CoinConiferColofulJackPotBar:updateSize()
...
function CoinConiferColofulJackPotBar:changeNode(label,index,isJump)
    local value=self.m_machine:BaseMania_updateJackpotScore(index)
    label:setString(util_formatCoinsLN(value,12,nil,nil,true))
end

return CoinConiferColofulJackPotBar