--[[
    ...
    关卡创建:
        self.m_symbolExpectCtr = util_createView("CodeCoinConiferSrc.CoinConiferSymbolExpect", self)
    以下接口需要在关卡内的直接重写:
        CoinConiferSymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        CoinConiferSymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        CoinConiferSymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        CoinConiferSymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        CoinConiferSymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local CoinConiferSymbolExpect = class("CoinConiferSymbolExpect")

function CoinConiferSymbolExpect:initData_(_machine)
    ...
    self.m_idleSymbolTypeList     = {                     --循环idle类型的图标列表
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER,
        self.m_machine.SYMBOL_BONUS_1,
        self.m_machine.SYMBOL_BONUS_2
    }
end

-- 接口在关卡内直接重写
--[[
function CoinConiferSymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function CoinConiferSymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function CoinConiferSymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function CoinConiferSymbolExpect:isReelRunSymbolType(_symbolType)
...
function CoinConiferSymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function CoinConiferSymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function CoinConiferSymbolExpect:isLoopIdleSymbol(_symbolType)
...
function CoinConiferSymbolExpect:MachineSymbolBulingEndCall(_slotNode)
...
--播放期待动画 
function CoinConiferSymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
        local maxRow = self.m_machine.m_iReelRowNum
        for iCol=1,_iCol do
            for iRow=1,maxRow do
...
function CoinConiferSymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function CoinConiferSymbolExpect:stopExpectAnim()
...
--播放循环idle
function CoinConiferSymbolExpect:playSymbolIdleAnim(_slotsNode)
...
return CoinConiferSymbolExpect