...
--smy
--2018年4月18日
--FarmBonus_WheelAction.lua


local FarmBonus_WheelAction = class("FarmBonus_WheelAction", util_require("base.BaseWheel"))

function FarmBonus_WheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function FarmBonus_WheelAction:changeWheelRunData( Array)
...
function FarmBonus_WheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function FarmBonus_WheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function FarmBonus_WheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function FarmBonus_WheelAction:recvData(index)
...
function FarmBonus_WheelAction:prepareStoping()
...
function FarmBonus_WheelAction:prepareBackFront(backV)
