...
--smy
--2018年4月18日
--JackpotOGoldWheelAction.lua


local JackpotOGoldWheelAction = class("JackpotOGoldWheelAction", util_require("base.BaseWheel"))

function JackpotOGoldWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function JackpotOGoldWheelAction:changeWheelRunData( Array)
...
function JackpotOGoldWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function JackpotOGoldWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function JackpotOGoldWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function JackpotOGoldWheelAction:recvData(index)
...
return JackpotOGoldWheelAction