local JungleJauntRespinTopReelNode = class("JungleJauntRespinTopReelNode",util_require("Levels.BaseReel.BaseReelNode"))
...
function JungleJauntRespinTopReelNode:ctor(params)
    JungleJauntRespinTopReelNode.super.ctor(self,params)
...
function JungleJauntRespinTopReelNode:createClipNode()
...
function JungleJauntRespinTopReelNode:startSchedule()
    --先加速再匀速后减速
...
function JungleJauntRespinTopReelNode:checkChangeSpeed(dt)
...
function JungleJauntRespinTopReelNode:getNextSymbolType()
...
function JungleJauntRespinTopReelNode:reloadRollNode(rollNode,rowIndex)

    self:removeSymbolByRowIndex(rowIndex)

    local symbolType = self:getNextSymbolType()
    local symbolNode = self.m_createSymbolFunc(symbolType, rowIndex, self.m_colIndex, self.m_isLastNode, self.m_lastNodeCount)
...
function JungleJauntRespinTopReelNode:removeSymbolFromRollNode(rollNode)
    ...
    if children and #children > 0 then
        for index = 1,#children do
            ...
            if self.m_pushSlotNodeToPoolFunc then
...
function JungleJauntRespinTopReelNode:updateRollNodePos(offset)
...
function JungleJauntRespinTopReelNode:getRollNodeMaxZOrder()
    local maxOrder = 0
    self:forEachRollNode(function(rollNode,bigRollNode,iRow)
        maxOrder = util_max(maxOrder,rollNode:getLocalZOrder())
    end)
    return maxOrder
end
...
function JungleJauntRespinTopReelNode:scaleRollNode(rollNode,rowIndex)
    local scale = 1
    ...
    if posX > minPosX and posX < maxPosX then
        local scaleRange     = self.m_parentData.slotNodeW / 2
        local centerDistance = math.abs(posX - self.m_parentData.reelWidth / 2)
        local offsetScale    = (1 - centerDistance / scaleRange) * 0.2
        scale = 1 + offsetScale
    end
...
return JungleJauntRespinTopReelNode