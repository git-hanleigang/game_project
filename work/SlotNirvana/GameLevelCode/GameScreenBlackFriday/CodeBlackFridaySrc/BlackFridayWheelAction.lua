...
--smy
--2018年4月18日
--BlackFridayWheelAction.lua


local BlackFridayWheelAction = class("BlackFridayWheelAction", util_require("base.BaseWheel"))

function BlackFridayWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function BlackFridayWheelAction:changeWheelRunData( Array)
...
function BlackFridayWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function BlackFridayWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function BlackFridayWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function BlackFridayWheelAction:recvData(index)
