local PirateBonusProgress = class("PirateBonusProgress", util_require("base.BaseView"))
-- 构造函数
local PROGRESS_WIDTH = 592
function PirateBonusProgress:initUI(data)
    local resourceFilename = "Pirate_bonus_jindutiao.csb"
    ...
    self.m_anim = util_createAnimation("Pirate_bonus_jindutiao_1.csb")
...
function PirateBonusProgress:lock(betLevel)
...
function PirateBonusProgress:unlock(betLevel)
    self.m_iBetLevel = betLevel
...
function PirateBonusProgress:idle()
...
--默认按钮监听回调
function PirateBonusProgress:clickFunc(sender)
...
function PirateBonusProgress:setPercent(percent, index)
...
function PirateBonusProgress:resetProgress(index, func)
    local percent = 100
...
function PirateBonusProgress:progressEffect(percent,isPlay)
...
function PirateBonusProgress:getCollectPos()
    local panda = self:findChild("pand_0")
    local pos = panda:getParent():convertToWorldSpace(cc.p(panda:getPosition()))
    return pos
end

function PirateBonusProgress:updatePercent(percent,callback)
    local oldPercent = self.m_progress:getPercent()
    ...
    self:runCsbAction("actionframe2", false, function()
        if percent >= 100 then
            ...
            --     self:completed()
            -- end, 2)
            gLobalSoundManager:playSound("PirateSounds/sound_Pirate_collect_completed.mp3")
...
function PirateBonusProgress:onEnter()

end

function PirateBonusProgress:onExit()

end

return PirateBonusProgress