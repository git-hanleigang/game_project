...
local MagikMirrorLongRunControl = class("MagikMirrorLongRunControl")

function MagikMirrorLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function MagikMirrorLongRunControl:getUsingReels()
    return self.usingReels
end

function MagikMirrorLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function MagikMirrorLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function MagikMirrorLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function MagikMirrorLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function MagikMirrorLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function MagikMirrorLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function MagikMirrorLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function MagikMirrorLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function MagikMirrorLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function MagikMirrorLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function MagikMirrorLongRunControl:setLongRunLenAndStates()
...
function MagikMirrorLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function MagikMirrorLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function MagikMirrorLongRunControl:clearCurLongRunData()
...
function MagikMirrorLongRunControl:setFirstColBeginLongRun()
...
return MagikMirrorLongRunControl
