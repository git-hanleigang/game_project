--[[
    ...
    以下接口需要在关卡内的直接重写:
        TurkeyDaySymbolExpect:checkSymbolTypePlayTipAnima
    以下接口需要插入关卡内的指定位置:
        TurkeyDaySymbolExpect:MachineSpinBtnCall             -> xxx:MachineRule_SpinBtnCall
        TurkeyDaySymbolExpect:MachineResetReelRunDataCall    -> xxx:MachineRule_ResetReelRunData
        TurkeyDaySymbolExpect:MachineOneReelDownCall         -> xxx:slotOneReelDown
        TurkeyDaySymbolExpect:MachineSymbolBulingEndCall     -> xxx:symbolBulingEndCallBack
        
]]
local TurkeyDaySymbolExpect = class("TurkeyDaySymbolExpect")

function TurkeyDaySymbolExpect:initData_(_machine)
...
-- 接口在关卡内直接重写
--[[
function TurkeyDaySymbolExpect:checkSymbolTypePlayTipAnima(symbolType)
    return false
end
]]

-- 接口插入关卡内 xxx:MachineRule_SpinBtnCall
function TurkeyDaySymbolExpect:MachineSpinBtnCall()
    --每次spin重置数据
    self.m_reelRunCol = 0
end

-- 接口插入关卡内 xxx:MachineRule_ResetReelRunData
function TurkeyDaySymbolExpect:MachineResetReelRunDataCall()
    --计算本次快滚图标类型
    self.m_reelRunSymbolTypeList  = {
        TAG_SYMBOL_TYPE.SYMBOL_SCATTER
    }
end
function TurkeyDaySymbolExpect:isReelRunSymbolType(_symbolType)
...
function TurkeyDaySymbolExpect:MachineOneReelDownCall(_iCol)
...
--快滚检测 
function TurkeyDaySymbolExpect:getNextReelLongRunState(_iCol)
...
--循环idle图标检测
function TurkeyDaySymbolExpect:isLoopIdleSymbol(_symbolType)
...
function TurkeyDaySymbolExpect:MachineSymbolBulingEndCall(_slotNode)
    local isPlay = true
...
--播放期待动画 
function TurkeyDaySymbolExpect:playExpectAnim(_iCol, _iRow)
    if not _iRow then
...
function TurkeyDaySymbolExpect:playSymbolExpectAnim(_slotsNode)
...
--停止期待动画 
function TurkeyDaySymbolExpect:stopExpectAnim()
...
--播放循环idle
function TurkeyDaySymbolExpect:playSymbolIdleAnim(_slotsNode, _isPlay)
    ...
    elseif _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS_1 then
        _slotsNode:runAnim("dan_idle2", true)
    elseif _slotsNode.p_symbolType == self.m_machine.SYMBOL_SCORE_BONUS_2 then
        _slotsNode:runAnim("dan_idleframe2", true)
    end
end

return TurkeyDaySymbolExpect