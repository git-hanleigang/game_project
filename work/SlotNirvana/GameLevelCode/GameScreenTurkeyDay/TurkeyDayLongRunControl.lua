...
local TurkeyDayLongRunControl = class("TurkeyDayLongRunControl")

function TurkeyDayLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function TurkeyDayLongRunControl:getUsingReels()
    return self.usingReels
end

function TurkeyDayLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function TurkeyDayLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function TurkeyDayLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function TurkeyDayLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function TurkeyDayLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function TurkeyDayLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function TurkeyDayLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function TurkeyDayLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function TurkeyDayLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function TurkeyDayLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function TurkeyDayLongRunControl:setLongRunLenAndStates()
...
function TurkeyDayLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function TurkeyDayLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function TurkeyDayLongRunControl:clearCurLongRunData()
...
function TurkeyDayLongRunControl:setFirstColBeginLongRun()
...
return TurkeyDayLongRunControl
