...
function JackPotNode:initFirstSymbolBySymbols(initDataList, reelHeight)
      ...
      -- self:addLayerColorBG(nil, 1000, 1000)
      ...
      for i=1, #initDataList do
            ...
            if i == 1 then
                posY = node.Height * self:getAnchorPointY()
            else 
...
function JackPotNode:getNextRunData(num)

      if type(self.m_runDataList) == "function" then
            return self.m_runDataList(num)
      else
...
function JackPotNode:createNextNode()
      if self:getLastSymbolTopY() >= self.m_reelHeight + 30 then
            --最后一个Node > 上边界 创建新的node  反之创建
            return 
      end
    
      
      gLobalSoundManager:playSound("PoseidonSounds/music_Poseidon_jackpot_reels_run.mp3")

      local nextNodeData = self:getNextRunData(self.m_jpNodeNum)
      if nextNodeData == nil then
          nextNodeData = self.m_randomDataFunc(self.m_jpNodeNum)
      end
      ...
      if nextNodeData.jpScore ~= nil then
            ...
            node.score =  xcyy.SlotsUtil:getArc4Random() % 9 + 1
      end
