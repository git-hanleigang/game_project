...
--smy
--2018年4月18日
--QuickSpinWheelAction.lua


local QuickSpinWheelAction = class("QuickSpinWheelAction", util_require("base.BaseWheel"))

function QuickSpinWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
    ...
    self.m_runV = 500--匀速
    self.m_runTime = 3 --匀速时间
    self.m_slowA = 300 --动态减速度
    self.m_slowQ = 3 --减速圈数
    self.m_stopV = 30--25 --停止时速度
...
function QuickSpinWheelAction:changeWheelRunData( Array)
...
function QuickSpinWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function QuickSpinWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function QuickSpinWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function QuickSpinWheelAction:recvData(index)
