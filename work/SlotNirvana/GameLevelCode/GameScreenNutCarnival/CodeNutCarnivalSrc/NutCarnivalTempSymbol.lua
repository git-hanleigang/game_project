local NutCarnivalTempSymbol = class("NutCarnivalTempSymbol", cc.Node)


-- 构造函数
function NutCarnivalTempSymbol:ctor()
...
function NutCarnivalTempSymbol:initData_(_initData)
    --初始化一些属性
    --[[
        _initData = {
            iCol       = 1,
...
function NutCarnivalTempSymbol:removeTempSlotsNode()
...
function NutCarnivalTempSymbol:changeSymbolCcb(_symbolType)
...
function NutCarnivalTempSymbol:createAnimNode()
...
function NutCarnivalTempSymbol:upDateAnimNode()
...
function NutCarnivalTempSymbol:checkLoadCCbNode()
    self:createAnimNode()
    return self.m_animNode
end
function NutCarnivalTempSymbol:getCCBNode()
    return self.m_animNode
end
--[[
    时间线相关
]]
function NutCarnivalTempSymbol:runAnim(animName,loop,func)
    self:createAnimNode()
    if not self.m_animNode then
        if func ~= nil then
            func()
        end
        local sMsg = strnig.format("[NutCarnivalTempSymbol:runAnim] m_symbolType=(%d)", self.m_symbolType)
        error(sMsg)
        return
    end
...
function NutCarnivalTempSymbol:registerAniamCallBackFun(_fun)
    self.m_fnRunAnimCallBack = _fun
end
--[[
    获取动画节点上面的子节点
]]
function NutCarnivalTempSymbol:getCcbProperty(propName)
...
function NutCarnivalTempSymbol:getAniamDurationByName(_animName)
    local mainClass = self.m_machine
    local spineSymbolData = mainClass.m_configData:getSpineSymbol(self.m_symbolType)
