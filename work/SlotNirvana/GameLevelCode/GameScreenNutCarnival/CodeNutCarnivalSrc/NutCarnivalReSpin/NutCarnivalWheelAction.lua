local NutCarnivalWheelAction = class("NutCarnivalWheelAction", util_require("base.BaseWheel"))

function NutCarnivalWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function NutCarnivalWheelAction:resetData()
    self.m_status = self.ACTION_READY
    self.m_isWheelData = false
    self.m_targetDistance = 0
    self.m_moveDistance = 0
...
function NutCarnivalWheelAction:changeWheelRunData( Array)
...
function NutCarnivalWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

function NutCarnivalWheelAction:recvData(index)
    self.m_isWheelData = true
    --最后停止的下标
    self.overIndex = index 
    --需要移动的距离
    self.m_moveDistance = 360-(self.overIndex - 1) * self.m_targetStep  
