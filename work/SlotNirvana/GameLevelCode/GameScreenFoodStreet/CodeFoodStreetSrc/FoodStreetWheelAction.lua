...
--smy
--2018年4月18日
--FoodStreetWheelAction.lua


local FoodStreetWheelAction = class("FoodStreetWheelAction", util_require("base.BaseWheel"))

function FoodStreetWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function FoodStreetWheelAction:changeWheelRunData( Array)
...
function FoodStreetWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function FoodStreetWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function FoodStreetWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function FoodStreetWheelAction:recvData(index)
