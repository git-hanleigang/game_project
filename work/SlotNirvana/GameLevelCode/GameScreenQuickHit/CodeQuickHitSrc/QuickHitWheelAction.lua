...
--smy
--2018年4月18日
--QuickHitWheelAction.lua


local QuickHitWheelAction = class("QuickHitWheelAction", util_require("base.BaseWheel"))

function QuickHitWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function QuickHitWheelAction:changeWheelRunData( Array)
...
function QuickHitWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function QuickHitWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function QuickHitWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function QuickHitWheelAction:recvData(index)
