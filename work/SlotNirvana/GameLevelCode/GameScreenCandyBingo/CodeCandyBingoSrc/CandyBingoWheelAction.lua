...
--smy
--2018年4月18日
--CandyBingoWheelAction.lua


local CandyBingoWheelAction = class("CandyBingoWheelAction", util_require("base.BaseWheel"))

function CandyBingoWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function CandyBingoWheelAction:changeWheelRunData( Array)
...
function CandyBingoWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function CandyBingoWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function CandyBingoWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end 

function CandyBingoWheelAction:recvData(index)
