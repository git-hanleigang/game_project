...
local ToroLocoLongRunControl = class("ToroLocoLongRunControl")

function ToroLocoLongRunControl:initData_(_machine)
...
-- 可以用这个接口来快速调快滚计算的假数据
function ToroLocoLongRunControl:getUsingReels()
    return self.usingReels
end

function ToroLocoLongRunControl:setUsingReels(_reels)
    self.usingReels = _reels
end

function ToroLocoLongRunControl:getLongRunInfoFromReel(_symbolTypes, _LegitimateCol, _LegitimateNum, _isContinuity)
...
function ToroLocoLongRunControl:getLongRun1toMaxColInfos(_symbolTypes, _legitimateNum)
...
function ToroLocoLongRunControl:getLongRun135ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function ToroLocoLongRunControl:getLongRun234ColInfos(_symbolTypes, _legitimateNum)
    -- 默认规则，一列只出同一个类型中的一个，两个以上开始快滚
...
function ToroLocoLongRunControl:getLongRunAnyNumAnyWhereInfos(_symbolTypes, _legitimateNum)
...
function ToroLocoLongRunControl:getLongRunAnyNumContinuityInfos(_symbolTypes, _legitimateNum)
...
function ToroLocoLongRunControl:getLongRunMustRunInfos(_symbolTypes, _legitimateNum, _musRunInfos)
    -- 默认规则，从左到右连续出现规定个数(默认四个)以上后，下一列就快滚，必须不能列中断出现
    local legitimateCol = {}
...
function ToroLocoLongRunControl:getLongRunStartAndEndColFromId(_longRunId, _symbolTypes, _legitimateNum, _musRunInfos)
    local infos = {}
...
function ToroLocoLongRunControl:getLongRunStartAndEndCol(_longRunConfigs)
    local longRunData = {}
    local longRunInfos = {}
...
function ToroLocoLongRunControl:setLongRunLenAndStates()
...
function ToroLocoLongRunControl:setCurLongRunData(_longRunData)
    self.m_curLongRunData = _longRunData
end

function ToroLocoLongRunControl:getCurLongRunData()
    return self.m_curLongRunData
end

function ToroLocoLongRunControl:clearCurLongRunData()
...
function ToroLocoLongRunControl:setFirstColBeginLongRun()
...
function ToroLocoLongRunControl:getScatterNumByCol(_col)
    local scatterNum = 0
    for iRow = 1, self.m_machine.m_iReelRowNum do
        for iCol = 1, _col do
