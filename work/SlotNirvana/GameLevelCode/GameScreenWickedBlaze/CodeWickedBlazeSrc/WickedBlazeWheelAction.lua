...
local WickedBlazeWheelAction = class("WickedBlazeWheelAction", util_require("base.BaseWheel"))

function WickedBlazeWheelAction:ctor(node, num, func,setRotFunc,Array)
    self.m_target = node
...
function WickedBlazeWheelAction:changeWheelRunData( Array)
...
function WickedBlazeWheelAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function WickedBlazeWheelAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function WickedBlazeWheelAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end

function WickedBlazeWheelAction:recvData(index,dirType)
