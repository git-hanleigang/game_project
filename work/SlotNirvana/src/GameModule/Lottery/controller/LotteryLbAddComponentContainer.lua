--[[
Author: cxc
Date: 2021-12-09 12:09:06
LastEditTime: 2021-12-09 12:10:09
LastEditors: cxc
Description: 乐透  金币增长动画管理类
FilePath: /SlotNirvana/src/GameModel/Lottery/controller/LotteryLbAddComponentContainer.lua
--]]
local LotteryLbAddComponentContainer = class("LotteryLbAddComponentContainer")

function LotteryLbAddComponentContainer:ctor()
...
function LotteryLbAddComponentContainer:updateData()
...
function LotteryLbAddComponentContainer:addComponent(_lb, _maxUIW, _limitStrCount)
    if not _lb then
        return
    end
...
function LotteryLbAddComponentContainer:updateSec()
    ...
    if #self.m_containerList <= 0 then
        self:setGrandPrize(self.m_showCoinsNumber)
        self:clearScheduler()
    end
end

function LotteryLbAddComponentContainer:initLabelUI(_info)
...
function LotteryLbAddComponentContainer:updateLabelUI(_info)
    if not next(_info) then
        return
    end
...
-- 获取金币滚动速率最大值等
function LotteryLbAddComponentContainer:getRateInfo(_pool)
...
-- 奖池金币数据 (存储)
function LotteryLbAddComponentContainer:setGrandPrize(_grandPrize)
...
-- 奖池金币数据 (获取)
function LotteryLbAddComponentContainer:getGrandPrize()
    local curTimeNumber = self.m_data:getCurTimeNumber()
    local beginCoins = gLobalDataManager:getNumberByField(self.m_grandPrizeSaveKey .. curTimeNumber, 0)
    return beginCoins
end

function LotteryLbAddComponentContainer:updateScheduler()
...
function LotteryLbAddComponentContainer:clearScheduler()
...
return LotteryLbAddComponentContainer
