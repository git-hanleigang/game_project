...
local MythicGameRewardBoxData = import(".MythicGameRewardBoxData")
local MythicGameRewardData = class("MythicGameRewardData")

function MythicGameRewardData:parseData(data)
    ...
    if data.rewards and #data.rewards > 0 then
        for i = 1, #data.rewards do
            local boxRewardData = MythicGameRewardBoxData:create()
...
-- 从0开始计数
function MythicGameRewardData:getBoxRewardIndexs()
    return self.p_boxRewardIndexs
end

function MythicGameRewardData:getLastBoxRewardIndex()
...
-- 从1开始计数, 客户端传给服务器的数值
function MythicGameRewardData:getOpenedClientPos()
    return self.p_openedClientPos
end

function MythicGameRewardData:getBoxRewards()
    return self.p_boxRewards
end

function MythicGameRewardData:getOpenedBoxCount()
    return #self.p_openedClientPos
end

function MythicGameRewardData:isFinish()
    return self.p_retainOpenCount == 0
end

function MythicGameRewardData:getBoxReward(_index)
...
function MythicGameRewardData:getWillOpenBoxRewardData()
...
function MythicGameRewardData:getUnOpenedBoxRewardData()
    local boxDatas = {}
    for i = 1, MythicGameConfig.BoxTotalCount do
...
function MythicGameRewardData:isWillOpenMonster()
    if self.p_boxRewards and #self.p_boxRewards > 0 then
        local index = self:getLastBoxRewardIndex()
        if index ~= nil then
...
function MythicGameRewardData:isBoxOpened(_boxIndex)
...
function MythicGameRewardData:checkPickAgain()
    if self:getOpenedBoxCount() == 1 and #self.p_boxRewardIndexs == 1 then
        local preOpenBoxRewardData = self:getWillOpenBoxRewardData()
        if preOpenBoxRewardData and not preOpenBoxRewardData:isMonsterBox() and not preOpenBoxRewardData:isMagicCard() then
            return true
        end
    end
    return false
