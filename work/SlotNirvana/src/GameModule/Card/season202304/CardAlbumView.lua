...
function CardAlbumView:initDatas(isPlayStart)
    ...
    self:setLandscapeCsbName(string.format(CardResConfig.seasonRes.CardAlbumViewRes, "season202304"))
    self:setName("CardAlbumView" .. "season202304")
end
...
function CardAlbumView:getTitleLuaPath()
    return "GameModule.Card.season202304.CardAlbumTitle"
end

function CardAlbumView:getBottomLuaPath()
    return "GameModule.Card.season202304.CardSeasonBottom"
end

function CardAlbumView:getCellLuaPath()
    return "GameModule.Card.season202304.CardAlbumCell"
end

function CardAlbumView:getMenuLuaPath()
    return "GameModule.Card.season202304.CardAlbumMenuLayer"
end
...
function CardAlbumView:getRoundGuideLuaPath()
    return "GameModule.Card.season202304.CardAlbumRoundGuide"
end
...
function CardAlbumView:playShowActionCustom(callFunc)
    ...
    local callback = function()
        ...
        self:runCsbAction(
            ...
            false,
            function()
                self:initRoundGuide() -- 引导从onEnter中挪到播完 CG 之后 202304
                if callFunc then
                    callFunc()
                end
...
function CardAlbumView:initSpineUI()
    if CardSysManager:isShowCG() then
        self.m_spineCG = util_spineCreate("CardRes/Other/spine/season202304_shoucijinru", true, true, 1)
        self.m_nodeSpineCG:addChild(self.m_spineCG)
    end
end

-- 进入集卡系统的开场动画 202304 新加的
...
function CardAlbumView:showAlbumMenuUI()
    if gLobalViewManager:getViewByName("CardAlbumMenuLayer" .. "season202304") ~= nil then
        return
    end
    local view = util_createView(self:getMenuLuaPath(), handler(self, self.showMenuCloseBtn))
    view:setName("CardAlbumMenuLayer" .. "season202304")
...
function CardAlbumView:clickFunc(sender)
    ...
    elseif name == "btn_menuClose" then
        ...
        local view = gLobalViewManager:getViewByName("CardAlbumMenuLayer" .. "season202304")
...
function CardAlbumView:getGuideLua()
    return "GameModule.Card.season202304.CardAlbumCellGuide"
end
