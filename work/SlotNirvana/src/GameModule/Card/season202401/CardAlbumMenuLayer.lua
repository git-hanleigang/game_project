...
function CardAlbumMenuLayer:initDatas(_startOver)
    ...
    self:setLandscapeCsbName("CardRes/season202401/cash_album_menu.csb")
    self.m_ruleLua = "GameModule.Card.season202401.CardMenuRule"
end
...
function CardAlbumMenuLayer:clickFunc(sender)
    ...
    if name == "Button_rules" then
        ...
        if gLobalViewManager:getViewByName("CardMenuRule" .. "season202401") ~= nil then
            return
        end
        local view = util_createView(self.m_ruleLua)
        view:setName("CardMenuRule" .. "season202401")
    ...
    elseif name == "Panel_menu" then
        local view = gLobalViewManager:getViewByName("CardAlbumView" .. "season202401")
        if view then
