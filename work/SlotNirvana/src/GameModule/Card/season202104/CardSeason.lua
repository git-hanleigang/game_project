...
function CardSeason:ctor()
    ...
    -- 卡册界面
    self.m_cardClanUI = "GameModule.Card.season202104.CardClanView"
    -- 卡册弹框
    self.m_bigCardUI = "GameModule.Card.season202104.BigCardLayer"
    -- nado卡获得进度界面
    self.m_linkProgress = "GameModule.Card.season202104.CardLinkProgressComplete"
    -- 章节完成界面
    -- self.m_clanCompleteLua = "GameModule.Card.season202104.CardClanComplete"
    self.m_statueCompleteLua = "GameModule.Card.season202104.CardStatueComplete"
...
function CardSeason:getClanCompleteCsbName(_clanId)
    ...
    if _clanData then
        if CardSysRuntimeMgr:isStatueClan(_clanData.type) then
            return string.format(CardResConfig.commonRes.statueClanComplete202104, "common" .. CardSysRuntimeMgr:getCurAlbumID())
        end
    end
    return CardSeason.super.getClanCompleteCsbName(self, _clanId)
    -- return string.format(CardResConfig.commonRes.clanComplete202104, "common" .. CardSysRuntimeMgr:getCurAlbumID())
end
...
function CardSeason:createCardSpecialGameBuffNode(buffMultiple)
    local statueBuffUI = util_createView("GameModule.Card.season202104.CardBuffTipNode", buffMultiple)
    return statueBuffUI
end
