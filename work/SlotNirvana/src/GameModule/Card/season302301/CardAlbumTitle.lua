...
function CardAlbumTitle:getCsbName()
    return string.format(CardResConfig.seasonRes.CardAlbumTitleRes, "season302301")
end

function CardAlbumTitle:getTimeLua()
    return "GameModule.Card.season302301.CardSeasonTime"
end

function CardAlbumTitle:getRoundTipLua()
    return "GameModule.Card.season302301.CardAlbumRoundTip"
end

function CardAlbumTitle:getRoundRewardListLua()
    return "GameModule.Card.season302301.CardAlbumRoundRewardList"
end
...
-- 新手集卡双倍奖励加成tagUI
function CardAlbumTitle:initDoubleRewardSignUI()
...
function CardAlbumTitle:getAlbumCoins()
    -- local data = G_GetMgr(G_REF.CardNovice):getData()
    -- if data then
    --     local albumData = data:getAlbumData()
    --     if albumData then
...
function CardAlbumTitle:getPro()
    ...
    --     local albumData = data:getAlbumData()
    --     if albumData then
    --         return albumData.current or 0, albumData.total or 5
    --     end
    -- end
    local albumData = CardSysRuntimeMgr:getCardAlbumInfo()
    if albumData then
