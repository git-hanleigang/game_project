...
function CardAlbumMenuLayer:initDatas(_startOver)
    ...
    self:setLandscapeCsbName("CardRes/season302301/cash_album_menu.csb")
    self.m_ruleLua = "GameModule.Card.season302301.CardMenuRule"
end
...
function CardAlbumMenuLayer:clickFunc(sender)
    ...
    if name == "Button_rules" then
        ...
        if gLobalViewManager:getViewByName("CardMenuRule" .. "season302301") ~= nil then
            return
        end
        local view = util_createView(self.m_ruleLua)
        view:setName("CardMenuRule" .. "season302301")
    ...
    elseif name == "Panel_menu" then
        local view = gLobalViewManager:getViewByName("CardAlbumView" .. "season302301")
        if view then
