...
function CardAlbumMenuLayer:initDatas(_startOver)
    ...
    self:setLandscapeCsbName("CardRes/season202303/cash_album_menu.csb")
    self.m_ruleLua = "GameModule.Card.season202303.CardMenuRule"
end
...
function CardAlbumMenuLayer:clickFunc(sender)
    ...
    if name == "Button_rules" then
        ...
        if gLobalViewManager:getViewByName("CardMenuRule" .. "season202303") ~= nil then
            return
        end
        local view = util_createView(self.m_ruleLua)
        view:setName("CardMenuRule" .. "season202303")
    ...
    elseif name == "Panel_menu" then
        local view = gLobalViewManager:getViewByName("CardAlbumView" .. "season202303")
        if view then
