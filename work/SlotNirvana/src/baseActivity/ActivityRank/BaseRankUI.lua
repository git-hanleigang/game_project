...
local BaseRankHelpUI = require("baseActivity.ActivityRank.BaseRankHelpUI")
local BaseRankTitleUI = require("baseActivity.ActivityRank.BaseRankTitleUI")
local BaseRankTimer = require("baseActivity.ActivityRank.BaseRankTimer")
local BaseRankUserCell = require("baseActivity.ActivityRank.BaseRankUserCell")
local BaseRankUserCell_Self = require("baseActivity.ActivityRank.BaseRankUserCell_Self")
local BaseRankRewardCell = require("baseActivity.ActivityRank.BaseRankRewardCell")
...
function BaseRankUI:initCsbNodes()
    ...
    self.userList = self:findChild("userList") -- 玩家列表容器
    assert(self.userList, "userList 挂点丢失")
    ...
    self.rewardList = self:findChild("rewardList") -- 奖励列表容器
...
function BaseRankUI:initUI()
    release_print("POKER CRASH[MQ], BaseRankUI:initUI 1")
    BaseRankUI.super.initUI(self)
    release_print("POKER CRASH[MQ], BaseRankUI:initUI 2")
    -- 打开界面请求，返回结果后刷新界面
    ...
    release_print("POKER CRASH[MQ], BaseRankUI:initUI 3")
end
...
--刷新界面
function BaseRankUI:updateView()
...
--刷新名次排行榜
function BaseRankUI:refreshRankList()
    local rank_data = self:getRankData()
    if not rank_data then
        return
    end
    ...
    for idx = 1, data_num do
        ...
        if not item then
            item = self:initRankCell(cell_data, idx, myIdx)
            if item then
                self.userList:insertCustomItem(item, idx - 1)
            end
        else
            if item.cell then
                if idx ~= myIdx and item.cell:getOpacity() == 0 then
                    item.cell:setOpacity(255)
                end
                item.cell:updateView(cell_data, idx, myIdx)
            end
        end
    end
    -- 去除多余条目
    ...
    local data_num = table.nums(list_data)
    if myIdx <= 0 or myIdx > data_num then
...
--刷新奖励排行榜
function BaseRankUI:refreshRewardList(data)
...
function BaseRankUI:clickFunc(sender)
    ...
    if name == "btn_rank" then
        ...
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_rank 1")
        self:showList(BTN_TYPE.RANK)
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_rank 2")
    elseif name == "btn_reward" then
        gLobalSoundManager:playSound(SOUND_ENUM.MUSIC_BTN_CLICK)
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_reward 1")
        self:showList(BTN_TYPE.REWARD)
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_reward 2")
    elseif name == "btn_rule" then
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_rule 1")
        self:showRankHelpUI()
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_rule 2")
    elseif name == "btn_close" then
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_close 1")
        self:closeUI()
        release_print("POKER CRASH[MQ], BaseRankUI:clickFunc btn_close 2")
    end
end
