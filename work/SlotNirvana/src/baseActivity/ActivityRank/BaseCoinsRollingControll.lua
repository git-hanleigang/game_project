...
function BaseCoinsRollingControll:getCoins_Record(act_key)
    local baseCoins = gLobalDataManager:getNumberByField(act_key, 0)
    return baseCoins
end
            ...
            function()
                ...
                for rolling_key, rolling_data in pairs(self.rolling_list) do
                    ...
                    if self:isRunning(act_ref) then
                        ...
                        --判断更新增量
                        local perAddCount = 0
                        if rolling_data.coins <= maxCoin then
                            perAddCount = perAdd * tick_timer
                        elseif rolling_data.coins < topMaxCoin then
                            perAddCount = topMaxperAdd * tick_timer
                        end

                        rolling_data.coins = rolling_data.coins + perAddCount
                        if rolling_data.coins >= topMaxCoin then
                            if rolling_data.bl_reset then
                                rolling_data.coins = baseCoin
                            else
                                rolling_data.coins = topMaxCoin
                            end
                        end
...
function BaseCoinsRollingControll:saveCoins(act_key, coins)
    if not act_key or not coins or coins < 0 then
        return
    end
    local coins = math.ceil(coins)
    gLobalDataManager:setNumberByField(act_key, coins)
end
