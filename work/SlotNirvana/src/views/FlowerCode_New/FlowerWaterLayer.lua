...
local FlowerWaterLayer = class("FlowerWaterLayer", BaseLayer)

function FlowerWaterLayer:initDatas(_type)
    self._type = _type
    ...
    self:setLandscapeCsbName("Flower/Activity/csd/EasterSeason_Operation/EasterSeason_Operation_new.csb")
    self:setPortraitCsbName("Flower/Activity/csd/EasterSeason_Operation/EasterSeason_Operation_veratical_new.csb")
...
function FlowerWaterLayer:updataFlowers()
    ...
    for i=1,#index_list do
        ...
        param.isPortrait = self:isPortraitWindow()
        local view = util_createView("views.FlowerCode_New.FlowerWaterItem",param)
        view:setName("WaterItem")
        if not globalData.slotRunData.isPortrait then
            view:setPositionY(-120)
        end
        node:addChild(view)
...
function FlowerWaterLayer:registerListener()
    gLobalNoticManager:addObserver(
        ...
        function(sender, _type)
            ...
            if #data > 0 then
                local view = util_createView("views.FlowerCode_New.FlowerBuyLayer",self._type)
    ...
    gLobalNoticManager:addObserver(
        ...
        function(sender, _num)
            ...
            param.type = self._type
            param.num = _num
            gLobalSoundManager:playSound("Flower/" ..self.config.SOUND.PAY)
...
function FlowerWaterLayer:createRewardLayer()
    ...
    local view = util_createView("views.FlowerCode_New.FlowerRewardLayer",itemList,clickFunc,coins,true,isBig)
    gLobalViewManager:showUI(view, ViewZorder.ZORDER_UI)
    performWithDelay(self, function()
        self:updataFlowers()
    end, 0.1)
    if isBig then
        gLobalSoundManager:playSound("Flower/" ..self.config.SOUND.REWARD)
    else
        gLobalSoundManager:playSound("Flower/" ..self.config.SOUND.REWARD1)
    end
end
...
function FlowerWaterLayer:resfuh(isBig)
    if isBig then
        if self.m_data:getRemainCoins() ~= 0 then
            ...
            local view = util_createView("views.FlowerCode_New.FlowerRewardLayer",item,cb,self.m_data:getRemainCoins(),true,3)
            gLobalViewManager:showUI(view, ViewZorder.ZORDER_UI)
        else
            gLobalNoticManager:postNotification(self.config.EVENT_NAME.NOTIFY_REWARD_BIG)
            self:removeFromParent()
...
function FlowerWaterLayer:openGuide()
    ...
    local guideLayer = util_createView("views.FlowerCode_New.FlowerGuideLayer",2)
