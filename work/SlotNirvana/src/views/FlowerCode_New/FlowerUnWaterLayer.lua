local FlowerUnWaterLayer = class("FlowerUnWaterLayer", BaseLayer)

function FlowerUnWaterLayer:initDatas()
    ...
    self:setLandscapeCsbName("Flower/Activity/csd/EasterSeason_mainUI/EasterSeason_mainUI_new.csb")
    self:setPortraitCsbName("Flower/Activity/csd/EasterSeason_mainUI/EasterSeason_mainUI_vertical/EasterSeason_mainUI_vertical_new.csb")
end

-- function FlowerUnWaterLayer:getCsbName()
--     local path = "Flower/Activity/csd/EasterSeason_mainUI/EasterSeason_mainUI_1.csb"
--     if globalData.slotRunData.isPortrait then
--         path = "Flower/Activity/csd/EasterSeason_mainUI/EasterSeason_mainUI_vertical/EasterSeason_mainUI_vertical_1.csb"
--     end
--     return path
-- end
...
function FlowerUnWaterLayer:updataSilver(_data)
    ...
    if not self.sl_FlowerLayer then
        self.sl_FlowerLayer = util_createView("views.FlowerCode_New.FlowerItem",1,self:isPortraitWindow())
        self.sl_FlowerLayer:setSpinePosY(60)
    ...
    if not self.sl_qipao then
        self.sl_qipao = util_createView("views.FlowerCode_New.FlowerQiPao",1)
...
function FlowerUnWaterLayer:updataGold(_data)
    ...
    if not self.gl_FlowerLayer then
        self.gl_FlowerLayer = util_createView("views.FlowerCode_New.FlowerItem",2,self:isPortraitWindow())
        self.gl_FlowerLayer:setSpinePosY(60)
    ...
    if not self.gl_qipao then
        self.gl_qipao = util_createView("views.FlowerCode_New.FlowerQiPao",2)
    ...
    gLobalNoticManager:addObserver(
        ...
        function(sender, _type)
            ...
            if not tolua.isnull(self.m_root) then
                ...
                local view = util_createView("views.FlowerCode_New.FlowerWaterLayer", _type)
                view:changeVisibleSize(_size)
                self:getParent():addChild(view)
            end
        end,
...
function FlowerUnWaterLayer:setQiPaoV(layer,layer_parent,_type)
    if layer and not tolua.isnull(layer) then
        ...
        local view = util_createView("views.FlowerCode_New.FlowerQiPao",_type)
...
function FlowerUnWaterLayer:openGuide()
    ...
    self.guideLayer = util_createView("views.FlowerCode_New.FlowerGuideLayer",1)
