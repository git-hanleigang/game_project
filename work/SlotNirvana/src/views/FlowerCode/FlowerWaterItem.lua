...
function FlowerWaterItem:initView()
    ...
    self.spot_anim = util_createView("views.FlowerCode.FlowerSpotAnim",param)
...
function FlowerWaterItem:playEndAnima(isBig)
    ...
    if isBig then
        csb_name = "click2"
        gLobalSoundManager:playSound(self.config.SOUND.HAPPY1)
    else
        gLobalSoundManager:playSound(self.config.SOUND.HAPPY)
    end
