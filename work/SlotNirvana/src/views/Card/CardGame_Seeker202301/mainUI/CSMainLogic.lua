...
function CSMainLogic:openSelectBox(_key, _over)
...
function CSMainLogic:seekPrize_playAddReward()
    ...
    self:playWinReward(self.m_cacheLevelIndex, function()
        if not tolua.isnull(self) then
            self:nextTriggerLogic("seek_prize")
        end
    end)
    ...
    util_performWithDelay(
        ...
        function()
            if not tolua.isnull(self) then
                ...
                local levelData = GameData:getCurLevelData()
                local boxRewards = levelData:getBoxRewards()
                for i = 1, CardSeekerCfg.BoxTotalCount do
                    -- 重置宝箱
                    local box = self.m_boxList[i]
                    ...
                    box:playStart(
                        function(_index)
                            if not tolua.isnull(box) and not tolua.isnull(self) then
                                box:resetView(boxRewards[_index])
                                self:setStatus("start_click", false)
                            end
                        end
                    )
