...
function CSMainBox:initView()
    ...
    if self.m_isOpened then
        -- if self.m_boxData:isMonsterBox() == true then
        --     self:playOpen(nil, "kong")
        -- else
...
function CSMainBox:initSpine()
    ...
    self.m_spNormalBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/ganlanqiu", true, true, 1)
    node_spine:addChild(self.m_spNormalBox)
    self.m_spSpecialBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/ganlanqiu2", true, true, 1)
    node_spine:addChild(self.m_spSpecialBox)
end

-- 更改为最初始的状态
function CSMainBox:resetView(_boxData)
    -- 更换层级
