...
function CSMainLayer:playWinReward(_index, _over)
    self.m_reward:winReward(_index)
    util_performWithDelay(self, _over, 1)
end

function CSMainLayer:initNpc()
    self.m_npc = util_spineCreate(CardSeekerCfg.otherPath .. "spine/npc", true, true, 1)
...
function CSMainLayer:initBoxes()
    ...
    for i = 1, CardSeekerCfg.BoxTotalCount do
        ...
        local isOpened = curLevelData:isBoxOpened(i)
        local boxData = boxRewardData[i]
        local box = util_createView(CardSeekerCfg.luaPath .. "mainUI.CSMainBox", i, special, isOpened, boxData, handler(self, self.clickBox))
...
function CSMainLayer:onKeyBack()
    if DEBUG == 2 then
        self:closeUI()
    end
end
...
function CSMainLayer:onShowedCallFunc()
    ...
    local willOpenIndexs = curLevelData:getBoxRewardIndexs()
    ...
    gLobalNoticManager:addObserver(
        ...
        function(target, params)
            ...
            if params and params.isSuc then
                ...
                -- if levelData:isWillOpenMonster() then
                --     -- self:resetBtnTake(false, "seek_monster")
                --     self:setBtnGoClickStatus(false)
                --     self:openSelectBox("seek_monster", function()
                --         self:doSeekMonsterLogic()
                --     end)
                -- else
                --     -- self:resetBtnTake(false, "seeker_prize")
                --     self:setBtnGoClickStatus(false)
                --     self:openSelectBox("seeker_prize", function()
                --         -- 新增再选一次
                --         -- 特殊关卡如果不是magic卡重新来一次
                --         if GameData:isCheckPickAgain(self.m_cacheLevelIndex) and levelData:checkPickAgain() then
                --             self:showPickAgainBtn(false)
                --         else
                --             -- 直接进入下一关
                --             gLobalSoundManager:playSound(CardSeekerCfg.otherPath .. "music/openPrize.mp3")
                --             self:doSeekPrizeLogic(false)
                --         end
                --     end)
                -- end
            end
