...
function CSMainLayer:initNpc()
    self.m_npc = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker202302_npc", true, true, 1)
...
function CSMainLayer:playNpcIdle()
    if self.m_npc then
        util_spinePlay(self.m_npc, "idle2", true)
    end
end
...
function CSMainLayer:clickBox(_index)
    ...
    if self.m_cacheLevelIndex == nil then
        self.m_logTrace = self.m_logTrace .. "|CSMainLayer:clickBox,_index=".._index..",m_cacheLevelIndex=nil"
    else
        self.m_logTrace = self.m_logTrace .. "|CSMainLayer:clickBox,_index=".._index..",m_cacheLevelIndex="..self.m_cacheLevelIndex
    end
...
function CSMainLayer:onShowedCallFunc()
    ...
    if openedIndexs and #openedIndexs > 0 then -- 当前层已经打开
        ...
        self.m_cacheLevelIndex = GameData:getCurLevelIndex()
        if self.m_cacheLevelIndex == nil then
            self.m_logTrace = self.m_logTrace .. "|CSMainLayer:onShowedCallFunc,m_cacheLevelIndex=nil"
        else
            self.m_logTrace = self.m_logTrace .. "|CSMainLayer:onShowedCallFunc,m_cacheLevelIndex="..self.m_cacheLevelIndex
        end        
