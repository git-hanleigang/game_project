...
function CSMainLogic:openSelectBox(_key)
    ...
    if box then
        ...
        if _key == "seek_monster" then
            ...
            local boxRewardSpine = self:createBoxReward(self.m_selectBoxIndex, willBoxData, false)
            util_spinePlay(boxRewardSpine, "idleframe", true)              
            box:playOpen(function()
                util_spinePlay(boxRewardSpine, "actionframe3", false)
                util_performWithDelay(self, function()
                    self:nextTriggerLogic(_key)
                end, 2)
            end, "start")
        else
            box:playOpen(nil, "start")
            ...
            if not tolua.isnull(self) then
                ...
                for i = 1, CardSeekerCfg.BoxTotalCount do
                    ...
                    if not isOpened then -- 未打开的箱子做动作
                        ...
                        box:playOpen(nil, "start")         
                        if boxData:isMonsterBox() then
                            local boxRewardSpine = self:createBoxReward(i, boxData, true)
                            util_spinePlay(boxRewardSpine, "idleframe1", true)
                        else
