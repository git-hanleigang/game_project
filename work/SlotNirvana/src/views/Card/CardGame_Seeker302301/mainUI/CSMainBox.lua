...
function CSMainBox:initSpine()
    self.m_spNormalBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker_card_normal", true, true, 1)
    self:findChild("Node_spine"):addChild(self.m_spNormalBox)
    self.m_spSpecialBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker_card_special", true, true, 1)
    self:findChild("Node_spine"):addChild(self.m_spSpecialBox)
end
...
function CSMainBox:initIcon()
    ...
    if self.m_levelType == CardSeekerCfg.LevelType.normal then
        self.m_spineCard = self.m_spNormalBox
    else
        self.m_spineCard = self.m_spSpecialBox
    end  
end
...
function CSMainBox:playStart(_over)
    ...
    -- util_spinePlay(self.m_spineCard, "start", false)
    -- util_spineEndCallFunc(self.m_spineCard, "start", function()
    --     if _over then
    --         _over()
    --     end
    -- end)
...
function CSMainBox:playUnOpendle()
    -- print("---CSMainBox:playUnOpendle ---", self.m_index, util_getymdhms_format(), debug.traceback("", 4))
    
    if self.m_index == 1 then
        gLobalSoundManager:playSound(CardSeekerCfg.otherPath .. "music/Seeker_gift_idle.mp3")
    end
    util_spinePlay(self.m_spineCard, "idle", true)
end
...
function CSMainBox:playOpenIdle()
    util_spinePlay(self.m_spineCard, "idle1", true)
end
