...
function CSMainLayer:initNpc()
    self.m_npc = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker_npc", true, true, 1)
...
function CSMainLayer:createBoxReward(_boxIndex, _boxData, _isGrey)
    ...
    if _boxData:isMonsterBox() then
        ...
        rewardNode = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker_emo", true, true, 1)
    else
    ...
    if rewardNode and box then
        ...
        util_spinePushBindNode(box_spine, "wupin", rewardNode)
...
function CSMainLayer:createGuoChane(_resetCall)
...
function CSMainLayer:initBoxReward()
    ...
    if openedIndexs and #openedIndexs > 0 then -- 当前层已经打开
        -- 上一次打开的是鲨鱼且没有操作，断线重连
        local openedBoxIndex = openedIndexs[#openedIndexs]
        if willOpenBoxData:isMonsterBox() then
            local boxRewardSpine = self:createBoxReward(openedBoxIndex, willOpenBoxData, false)
            util_spinePlay(boxRewardSpine, "idleframe", true)
        end
    end
end
...
function CSMainLayer:initBoxes()
    ...
    for i = 1, CardSeekerCfg.BoxTotalCount do
        ...
        local isOpened = curLevelData:isBoxOpened(i)
        local box = util_createView(CardSeekerCfg.luaPath .. "mainUI.CSMainBox", i, special, isOpened, boxRewardData[i],
            handler(self, self.clickBox))
