...
function CSMainProgress:initBalls()
    ...
    for i = 1, count do
        ...
        local ballY = ballFirstPosY - ballInterval * (i - 1)
        ball:setPosition(cc.p(0, ballY))
        table.insert(self.m_balls, ball)
        table.insert(self.m_ballYs, ballY)
    end
    local ballsY = self.m_oriPosY + ballInterval * (curIndex - 1)
...
function CSMainProgress:moveBalls(_moveOver)
    ...
    actionList[#actionList + 1] =
        ...
        cc.MoveTo:create(0.5, cc.p(posX, posY + ballInterval))
    )
    actionList[#actionList + 1] =
        cc.CallFunc:create(
        function()
