...
function CSLoseLayer:initCsbNodes()
    self.m_nodeMonster = self:findChild("spine_shayu")
...
function CSLoseLayer:initView()
    ...
    performWithDelay(
        self,
        function()
            self.m_monster = util_spineCreate(CardSeekerCfg.otherPath .. "spine/shayu", true, true, 1)
            self.m_nodeMonster:addChild(self.m_monster)
            gLobalSoundManager:playSound(CardSeekerCfg.otherPath .. "music/lose_monster.mp3")
            util_spinePlay(self.m_monster, "start", false)
