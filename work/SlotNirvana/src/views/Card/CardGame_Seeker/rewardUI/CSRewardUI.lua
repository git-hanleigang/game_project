...
function CSRewardUI:initDatas(_isFinal, _over)
    ...
    if _isFinal == true then
        self:setLandscapeCsbName(CardSeekerCfg.csbPath .. "Seeker_RewardLayer_Final.csb")
    else
...
function CSRewardUI:initView()
    gLobalSoundManager:playSound(CardSeekerCfg.otherPath .. "music/reward.mp3")
    self:initDropList()
    self:initRewards()
...
function CSRewardUI:initRewards()
    ...
    local coinNum = winData:getCoins() or 0
    local gemNum = winData:getGems() or 0
    local items = winData:getMergeItems()
...
function CSRewardUI:flyCoins(_flyCoinsEndCall)
    ...
    local coins = winData:getCoins() or 0
    local gems = winData:getGems() or 0
...
function CSRewardUI:getTSGameData()
