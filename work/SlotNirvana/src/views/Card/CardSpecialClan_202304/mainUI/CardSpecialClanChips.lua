...
function CardSpecialClanChips:getCsbName()
    return "CardRes/" .. G_GetMgr(G_REF.CardSpecialClan):getThemeName() .. "/csb/main/MagicClanChips.csb"
end

function CardSpecialClanChips:initCsbNodes()
    self.m_nodeChips = {}
    for i = 1, CardSpecialClanCfg.QuestMagicClanCardNum do
...
function CardSpecialClanChips:initCells()
    ...
    local pageCards = clanData:getPageCards(1) -- 只有一页
    self.m_chips = {}
    for i = 1, #self.m_nodeChips do
        ...
        if cardData then
            ...
            chip:playAnimByIndex(i, self.m_nodeChips[i], true, nil, CardSpecialClanCfg.QuestMagicClanCardNum)
...
function CardSpecialClanChips:updateCells(_pageIndex, _over)
    ...
    local cardDatas = clanData:getPageCards(1) -- 只有一页
    for i = 1, #self.m_chips do
        -- local cardData = cards[i + CardSpecialClanCfg.QuestMagicClanCardNum * (self.m_pageIndex - 1)]
        ...
        if cardData then
            ...
            self.m_chips[i]:playAnimByIndex(
                ...
                CardSpecialClanCfg.QuestMagicClanCardNum
            )
        end
    end
end
...
function CardSpecialClanChips:checkCardNewMark()
    ...
    local pageNewCards = clanData:getPageNewCards(1) -- 只有一页
...
function CardSpecialClanChips:getClanData()
    ...
    if data then
        local clanData = data:getSpecialClanByIndex(self.m_pageIndex)
        return clanData
    end
    return nil
end
