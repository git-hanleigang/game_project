...
function CSInfoLayer:initCsbNodes()
    ...
    for i = 1, math.huge do
        local bgLights = self:findChild("node_bg_light_" .. i)
        if not bgLights then
            break
        end
        table.insert(self.m_nodeBgLights, bgLights)
    end
...
function CSInfoLayer:hideBgLights()
    for i = 1, #self.m_nodeBgLights do
        if self.m_nodeBgLights[i] then
            self.m_nodeBgLights[i]:setVisible(false)
        end
    end
end
