...
function CSMainBox:initView()
    ...
    self:runCsbAction("idle", true, _over, 60)
end

function CSMainBox:initSpine()
    local node_spine = self:findChild("Node_spine")
    self.m_spNormalBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker202401_Box_normal", true, true, 1)
    node_spine:addChild(self.m_spNormalBox)
    self.m_spSpecialBox = util_spineCreate(CardSeekerCfg.otherPath .. "spine/Seeker202401_Box_special", true, true, 1)
...
function CSMainBox:playStart(_over)
    -- print("---CSMainBox:playStart ---", self.m_index)
...
function CSMainBox:playShake(_over)
    -- print("---CSMainBox:playShake ---", self.m_index)
    self:runCsbAction("shake", false, _over, 60)
end

function CSMainBox:playShowIdle()
    -- print("---CSMainBox:playShowIdle ---", self.m_index)
    if self.m_index == 1 then
        gLobalSoundManager:playSound(CardSeekerCfg.otherPath .. "music/Seeker_gift_idle.mp3")
    end
    util_spinePlay(self.m_nowBox, "idle", true)
end

function CSMainBox:playKongIdle()
    util_spinePlay(self.m_nowBox, "kong", true)
end

function CSMainBox:playHideIdle()
    -- print("---CSMainBox:playHideIdle ---", self.m_index)
    --self:runCsbAction("idle_hide", true, nil, 60)
end

function CSMainBox:playOpen(_over, _isClick, _isMonster)
    -- print("---CSMainBox:playOpen ---", self.m_index)
    local key = nil
    if _isMonster then
        if _isClick then
            key = "open_guai_click"
        else
            key = "open_guai_noclick"
        end
    else
        if _isClick then
            key = "open_reward_click"
        else
            key = "open_reward_noclick"
        end
    end
    util_spinePlay(self.m_nowBox, key, false)
    util_spineEndCallFunc(self.m_nowBox, key, function()
        if _over then
            _over()
        end
    end)
...
function CSMainBox:playHide(_over)
    -- print("---CSMainBox:playHide ---", self.m_index)
