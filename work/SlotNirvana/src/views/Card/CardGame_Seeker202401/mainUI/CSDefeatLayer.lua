...
function CSDefeatLayer:initCsbNodes()
    ...
    self.m_nodeFirstBuy = self:findChild("Node_1STFREE")
    
    self.m_btnGem = self:findChild("btn_Gem")
    self:setButtonLabelContent("btn_Gem", "")

    -- 月卡临时设置 ------------------------------------------------
    self.m_nodebtnGem = self:findChild("node_btnGem")
    self.m_nodebtnGem:setPositionX(-10)

    self.m_btnMonthlyCard = self:findChild("btn_MonthlyCard")
    
    self.m_nodeMonthlyCard = self:findChild("node_btnMonthlyCard")
    self.m_nodeMonthlyCard:setVisible(false)
    self:setButtonLabelContent("btn_MonthlyCard", "")       -- 2024.01 新增 屏蔽按钮文字 "collect"
    -- 月卡临时设置 end---------------------------------------------

    self.m_nodeBtnContinue = self:findChild("node_btnContinue")
    self.m_nodeBtnFree = self:findChild("node_btnFree")

    self.m_panelGiveup = self:findChild("Panel_giveup")
    self.m_spseek = self:findChild("seek_sm")
    self.m_lbseek = self:findChild("seek_lbgem")
    self.m_loser = self:findChild("seek_hs")
...
-- 宝石不足的红色提示
function CSDefeatLayer:initGemLose()
    local GameData = self:getTSGameData()
    if not GameData then
        return
    end
...
function CSDefeatLayer:initFirstBuy()
    if self:isFirstBuy() then
        local freeTip = util_createAnimation(CardSeekerCfg.csbPath .. "Seeker_Bubble_1stFree.csb")
        self.m_nodeFirstBuy:addChild(freeTip)
        freeTip:playAction(
            "start",
            false,
            function()
...
function CSDefeatLayer:initPrice()
    ...
    self.m_lbGem:setString(costGem)
    util_alignCenter({
        {node = self.m_spGem, scale = 0.8},
        {node = self.m_lbGem, scale = 1, alignX = 5}
    }, nil, 360)
end

function CSDefeatLayer:initBtnGemText()
    self.m_nodeBtnContinue:setVisible(true)
    self.m_nodeBtnFree:setVisible(false)

    if self:isFirstBuy() then
        self.m_nodeBtnContinue:setVisible(false)
        self.m_nodeBtnFree:setVisible(true)
    elseif self:isMonthlyFree() then
        ...
        if isFirstEntrySharkGame then
            ...
            actionList[#actionList + 1] = cc.CallFunc:create(function()
                self.m_nodeBtnContinue:setVisible(false)
                self.m_nodeBtnFree:setVisible(true)
                self.m_nodeBtnFree:setOpacity(0)
                self.m_nodeBtnFree:runAction(cc.FadeTo:create(20 / 60, 255))
            end)
            self.m_nodeBtnContinue:runAction(cc.Sequence:create(actionList))            
        else
            self.m_nodeBtnContinue:setVisible(false)
            self.m_nodeBtnFree:setVisible(true)
        end
...
-- 202401 新增加播放start动画
function CSDefeatLayer:playShowAction()
    gLobalSoundManager:playSound("Sounds/soundOpenView.mp3")
    CSDefeatLayer.super.playShowAction(self, "start")
end
...
function CSDefeatLayer:clickFunc(sender)
    ...
    if name == "btn_Gem" then
        ...
        if self:isFirstBuy() or self:isMonthlyFree() then
...
function CSDefeatLayer:isFirstBuy()
...
function CSDefeatLayer:isMonthlyFree()
    local GameData = self:getTSGameData()
    if GameData then
