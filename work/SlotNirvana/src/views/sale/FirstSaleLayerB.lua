...
local FirstSaleLayerB = class("FirstSaleLayerB", BaseLayer)

function FirstSaleLayerB:initDatas(_params, _data)
    self.m_saleData = _data
    self.m_backupData = clone(_data)
...
function FirstSaleLayerB:initCsbNodes()
    ...
    self.m_node_price = self:findChild("lb_node_price")

    self.m_title_1 = self:findChild("sp_title1")
    self.m_title_2 = self:findChild("sp_title2")

    self.m_node_promo = self:findChild("node_promo")
    self.m_lb_promo = self:findChild("lb_promo")
end

function FirstSaleLayerB:initView()
    self:initTitle()
    self:initPrice()
    self:updateTime()
    self:initBubbleNode()
...
function FirstSaleLayerB:initTitle()
    local bagType = self.m_saleData:getBagType()
    self.m_title_1:setVisible(bagType == 1)
    self.m_title_2:setVisible(bagType == 2)
end

function FirstSaleLayerB:initPrice()
    ...
    if self.m_saleData.p_fakePrice and self.m_saleData.p_fakePrice ~= "" then
        local priceNode = util_createView("views.sale.FirstSaleB_NoCoinsPrice", self.m_saleData)
        if priceNode then
            self.m_node_price:addChild(priceNode)
        end

        self:setButtonLabelContent("btn_buy", "")
    else
        local price = self.m_saleData.p_price -- 本次首充价格
        ...
        self:setButtonLabelContent("btn_buy", refStr .. price)
    end
    -- self.m_label_newprice:setString("$" .. self.m_saleData.p_price)
...
function FirstSaleLayerB:initBubbleNode()
...
-- function FirstSaleLayerB:initSpine()
--     local csbTheme = "Promotion/FirstCommonSale_normal"
--     if globalData.constantData.FIRST_COMMON_SALE_SPECIAL_THEME then
--         csbTheme = "Promotion/FirstCommonSale_special"
--     end 
...
function FirstSaleLayerB:buySuccess()
    --购买成功提示界面
    self.m_isBuy = true
    ...
    -- 需要删掉数据
    if self.m_saleData:isLastBag() and self.m_saleData:isOnSale() then
        G_GetMgr(G_REF.FirstCommonSale):deleteFirstSaleData()
    end

    gLobalNoticManager:postNotification(ViewEventType.NOTIFY_FIRST_SALE_BUYSUCCESS)

    view:initBuyTip(buyType, self.m_backupData, self.m_backupData.p_originalCoins, levelUpNum)
...
function FirstSaleLayerB:buyFailed(_errorInfo)
...
-- 检查是否弹出 二次确认弹板
function FirstSaleLayerB:checkPopPayConfirmLayer(_errorInfo)
...
function FirstSaleLayerB:onKeyBack()
...
function FirstSaleLayerB:clickFunc(sender)
...
function FirstSaleLayerB:onEnter()
    FirstSaleLayerB.super.onEnter(self)
    if not self.m_saleData then
        return
    end

    local goodsInfo = {}
    goodsInfo.goodsTheme = "FirstSaleLayerB"
    ...
    -- purchaseInfo.purchaseName = "FirstSuperSaleB_" .. tostring(self.m_saleData:getBagId())
    purchaseInfo.purchaseName = "FirstSuperSale"
    purchaseInfo.purchaseStatus = tostring(self.m_saleData:getBagId())

    gLobalSendDataManager:getLogIap():openIapLogInfo(goodsInfo, purchaseInfo, nil, nil, self)
end

function FirstSaleLayerB:closeUI(isLog, resultData)
    if self.isClose then
        return
    end
    self.isClose = true
    ...
    FirstSaleLayerB.super.closeUI(self, callBack)
end

function FirstSaleLayerB:registerListener()
    FirstSaleLayerB.super.registerListener(self)
...
function FirstSaleLayerB:playShowAction()
    if globalData.constantData.FIRST_COMMON_SALE_SPECIAL_THEME then
        -- 特殊的使用通用弹板弹出动画
        self:runCsbAction("idle", true, nil, 60)
        FirstSaleLayerB.super.playShowAction(self)
        return
    end
    ...
    FirstSaleLayerB.super.playShowAction(self, userDefAction)
end

function FirstSaleLayerB:onShowedCallFunc()
    self:runCsbAction("idle", true, nil, 60)
...
function FirstSaleLayerB:getUnLock()
    local isf = false
...
-- 获取限时 道具
function FirstSaleLayerB:getShowSaleDataItems()
