local BasicSaleWheelControl = class("BasicSaleWheelControl", util_require("base.BaseWheel"))

function BasicSaleWheelControl:ctor(node, num, func,setRotFunc,Array,bindKey)
    self.m_target = node
    ...
    -- 忽略basewheel 中的轮盘暂停判断
    self:setIgnorePause(true)
end

function BasicSaleWheelControl:changeWheelRunData( Array)
    self.m_status = self.ACTION_READY
...
function BasicSaleWheelControl:setEndFunc(_func)
    self.m_func = _func
end

function BasicSaleWheelControl:getEndFunc()
    return self.m_func
end

function BasicSaleWheelControl:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function BasicSaleWheelControl:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function BasicSaleWheelControl:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end

function BasicSaleWheelControl:recvData(index,dirType)
