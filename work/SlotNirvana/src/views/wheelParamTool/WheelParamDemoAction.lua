--轮盘转动界面
--WheelParamDemoAction.lua

local WheelParamDemoAction = class("WheelParamDemoAction", util_require("base.BaseWheel"))

function WheelParamDemoAction:ctor(node, num, func,setRotFunc,Array,bindKey)
    self.m_target = node
...
function WheelParamDemoAction:changeWheelRunData( Array)
...
function WheelParamDemoAction:setWheelRotFunc( setRotFunc )
    self.m_setRotFunc = setRotFunc
end

--保存时间
function WheelParamDemoAction:saveTime()
    self.m_curTime = socket.gettime()
end

--读取时间间隔
function WheelParamDemoAction:getSpanTime()
    local spanTime = (socket.gettime() - self.m_curTime)
    return spanTime
end

function WheelParamDemoAction:recvData(index,dirType)
