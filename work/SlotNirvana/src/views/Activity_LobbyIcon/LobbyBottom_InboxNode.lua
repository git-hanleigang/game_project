...
local LobbyBottom_InboxNode = class("LobbyBottom_InboxNode", BaseLobbyNodeUI)
LobbyBottom_InboxNode.m_isChangeParent = nil -- 因为引导原因提高层级
LobbyBottom_InboxNode.m_inboxGuideList = nil
-- 节点特殊ui 配置相关 --
function LobbyBottom_InboxNode:initUI(data)
...
-- function LobbyBottom_InboxNode:initView( )

-- end

function LobbyBottom_InboxNode:updateView()
    -- 单纯重写 防止父类调用
...
function LobbyBottom_InboxNode:getBottomName()
    return "INBOX"
end

function LobbyBottom_InboxNode:refreshInboxTip(params)
...
--更新特殊气泡
function LobbyBottom_InboxNode:checkShowRewardTips()
        ...
        if not self.m_isChangeParent then
            ...
            self.m_newbieMask:onTouch(
                function(event)
                    --点击消失
                    -- self:hideRewardTips()
                    gLobalNoticManager:postNotification("hide_reward_tip_inbox")
                end
            )
        end
        --5秒小时
...
function LobbyBottom_InboxNode:hideRewardTips()
    --还原层级
...
--检测是否有RepartReward
function LobbyBottom_InboxNode:checkRepartReward(inboxType)
...
-- 节点特殊处理逻辑 --
function LobbyBottom_InboxNode:clickLobbyNode()
    --
...
function LobbyBottom_InboxNode:getGameData()
    -- 这是不需要活动数据的关卡 直接返回nil
    return nil
end

-- onEnter
function LobbyBottom_InboxNode:onEnter()
