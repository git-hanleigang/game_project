local UserInfoFirstItem = class("UserInfoFirstItem", BaseLayer)
local ClanManager = util_require("manager.System.ClanManager"):getInstance()

function UserInfoFirstItem:ctor()
    UserInfoFirstItem.super.ctor(self)
    self:setExtendData("UserInfoFirstItem")
    self:setLandscapeCsbName("Activity/csd/Information/Iformation_Zong/Iformation_zong_no_trophy.csb")
...
function UserInfoFirstItem:initView()
...
function UserInfoFirstItem:updataHistory()
...
function UserInfoFirstItem:registerListener()
...
function UserInfoFirstItem:updataVip()
...
function UserInfoFirstItem:updataTeam()
    ...
    if clanData then
        ...
        if clanSimpleInfo and clanSimpleInfo.getTeamLogo then
            ...
            if not _iconName or _iconName == "" then
                _iconName = 0
            end
...
function UserInfoFirstItem:updataFrame()
...
function UserInfoFirstItem:updataSession()
    ...
    local data = G_GetMgr(G_REF.LeagueCtrl):getOpenCtrl():getData()
    if data then
...
function UserInfoFirstItem:clickStartFunc(sender)
end

function UserInfoFirstItem:clickFunc(sender)
