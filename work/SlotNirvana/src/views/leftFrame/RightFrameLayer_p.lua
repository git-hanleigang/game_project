...
function RightFrameLayer:initUI(worldPos)
    ...
    self:createCsbNode("RightFrame/RightFrameNode.csb")
    -- 创建一个基准 baseNode
...
function RightFrameLayer:updateBGPanel(size)
    local height = math.max(180, size.height + 30)
    self.m_bg:setContentSize(110, height)
    self.m_panel:setContentSize(110, height)
    self.m_node_btn:setPositionY(height * 0.5)

    if table.nums(self.m_realExtraNodeInfo) == 1 then
        -- 一个的时候太小了 背景 扩大入口居中
        if self.m_realExtraNodeInfo[1] and not tolua.isnull(self.m_realExtraNodeInfo[1].node) then
            self.m_realExtraNodeInfo[1].node:setPositionY(height * 0.5 - 16)
        end
    end
