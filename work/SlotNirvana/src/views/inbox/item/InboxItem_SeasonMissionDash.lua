local InboxItem_SeasonMissionDash = class("InboxItem_SeasonMissionDash", util_require("views.inbox.item.InboxItem_baseReward"))
local ShopItem = require "data.baseDatas.ShopItem"

function InboxItem_SeasonMissionDash:getCardSource()
    return {"Pass Task Extra"}
end

function InboxItem_SeasonMissionDash:getCsbName()
    return "InBox/InboxItem_Common_Reward.csb"
end

function InboxItem_SeasonMissionDash:getDescStr()
...
function InboxItem_SeasonMissionDash:initData()
    InboxItem_SeasonMissionDash.super.initData(self)
    self:initDropNextEvent()
end

function InboxItem_SeasonMissionDash:initDropNextEvent()
    local _doFuncList = {}
...
-- 检测 list 调用方法
function InboxItem_SeasonMissionDash:triggerNextFunc()
...
-- 检测掉落卡牌
function InboxItem_SeasonMissionDash:triggerDropCards()
    local this = self
...
-- 检测高倍场体验卡
function InboxItem_SeasonMissionDash:triggerDeluxeCard()
    local this = self
...
-- 检测掉落 合成福袋
function InboxItem_SeasonMissionDash:triggerMergeBagView()
    local this = self
...
function InboxItem_SeasonMissionDash:getMergeBagList()
    local list = {}
...
function InboxItem_SeasonMissionDash:collectMailSuccess()
...
function InboxItem_SeasonMissionDash:removeSelfItem()
    if not self.m_dropFuncList or #self.m_dropFuncList <= 0 then
        InboxItem_SeasonMissionDash.super.removeSelfItem(self)
        gLobalNoticManager:postNotification(ViewEventType.NOTIFY_INBOX_UPDATE_LOCAL_MAIL)
    end
end


return InboxItem_SeasonMissionDash