...
local InboxItem_holidayChallenge = class("InboxItem_holidayChallenge", InboxItem_baseReward)

function InboxItem_holidayChallenge:getCsbName()
    return "InBox/InboxItem_Common_Reward.csb"
end

-- 描述说明
function InboxItem_holidayChallenge:getDescStr()
    return self.m_mailData.title or "SPRING EGG HUNT REWARDS"
end

function InboxItem_holidayChallenge:collectMailSuccess()
    self:gainRewardSuccess()
    self:removeSelfItem()
end

-- 领取成功
function InboxItem_holidayChallenge:gainRewardSuccess()
    -- 需要做一个通用弹板，因为后面需要
...
return  InboxItem_holidayChallenge