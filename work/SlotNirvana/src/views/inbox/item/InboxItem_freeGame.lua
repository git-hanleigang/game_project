...
local InboxItem_freeGame = class("InboxItem_freeGame", util_require("views.inbox.item.InboxItem_base"))

function InboxItem_freeGame:getCsbName()
    -- 默认皮肤
...
function InboxItem_freeGame:initView()
    self:readNodes()
    self:updateUI()
    self:updateTime()
end

function InboxItem_freeGame:readNodes()
...
function InboxItem_freeGame:updateUI()
    -- 数据无效 移除
    local freeSpinData = globalData.iapRunData:getFreeGameData()
    local ticketData = freeSpinData:getRewardsById( self.m_mailData.ticketId )
...
function InboxItem_freeGame:setButtonEnabled()
    local freeSpinData = globalData.iapRunData:getFreeGameData()
    local ticketData = freeSpinData:getRewardsById( self.m_mailData.ticketId )
    ...
    if self.m_btn_collect then
        ...
        self.m_btn_collect:getParent():setVisible( not ticketData:isActive() )
    end
    if self.m_btn_continue then
        self:setButtonLabelDisEnabled("btn_continue", enabled)
        self.m_btn_continue:getParent():setVisible( ticketData:isActive() )
    end
end

function InboxItem_freeGame:updateTime()
    --无数据或者过期了
    local freeSpinData = globalData.iapRunData:getFreeGameData()
    local ticketData = freeSpinData:getRewardsById( self.m_mailData.ticketId )
    if not ticketData or ticketData:isOverdue() then
...
function InboxItem_freeGame:clickFunc(sender)
    -- 如果不是在大厅场景内 不响应
...
function InboxItem_freeGame:sendCollectMail()
    if not self:checkLevelEnable() then
        return
    end
    ...
    gLobalSendDataManager:getNetWorkFeature():sendFreeGameActive(ticketId,
...
function InboxItem_freeGame:checkLevelEnable()
    if not gLobalViewManager:isLobbyView() then
        return
    end
...
--进入关卡
function InboxItem_freeGame:enterLevel()
    if not self:checkLevelEnable() then
        return
    end
...
function InboxItem_freeGame:collectMailFail()
end

function InboxItem_freeGame:hideTicket()
    if self.isHide then
        return
    end
    self.isHide = true
    G_GetMgr(G_REF.Inbox):getDataMessage(function()
...
function InboxItem_freeGame:removeSelfItem()
...
function InboxItem_freeGame:onExit()
    if self.m_schedule then
        self:stopAction(self.m_schedule)
        self.m_schedule = nil
    end
...
-- function InboxItem_freeGame:getExpireTime()
--     --无数据或者过期了
--     local freeSpinData = globalData.iapRunData:getFreeGameData()
--     local ticketData = freeSpinData:getRewardsById( self.m_mailData.ticketId )
--     if not ticketData or ticketData:isOverdue() then
