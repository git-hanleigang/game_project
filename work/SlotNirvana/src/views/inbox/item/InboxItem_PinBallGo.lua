...
local InboxItem_pinballgo = class("InboxItem_pinballgo", InboxItem_base)

function InboxItem_pinballgo:getCsbName( )
      return "InBox/InboxItem_PinballGo.csb"
end
-- 描述说明
function InboxItem_pinballgo:getDescStr()
      return "PINBALL GO", "Play and win big prizes!"
end
-- -- 结束时间(单位：秒)
-- function InboxItem_pinballgo:getExpireTime()
...
-- end
-- 倒计时结束回调
function InboxItem_pinballgo:timeEndCallback()
      local levelDashData = G_GetActivityDataByRef(ACTIVITY_REF.LevelDash)
      levelDashData:setLevelDashStatus(LEVEL_DASH_STATUS.WAIT)
end

function InboxItem_pinballgo:clickFunc(sender)
