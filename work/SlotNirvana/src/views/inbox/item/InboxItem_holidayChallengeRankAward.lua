...
local InboxItem_holidayChallengeRankAward = class("InboxItem_holidayChallengeRankAward", InboxItem_base)

function InboxItem_holidayChallengeRankAward:getCsbName()
    return "InBox/InboxItem_Common_Reward.csb"
end

-- 描述说明
function InboxItem_holidayChallengeRankAward:getDescStr()
    return  self.m_mailData.title or "EGG RANKING REWARDS"
end

function InboxItem_holidayChallengeRankAward:collectMailSuccess()
    self:gainRewardSuccess()
    self:removeSelfItem()
end

-- 领取成功
function InboxItem_holidayChallengeRankAward:gainRewardSuccess()
    -- 需要做一个通用弹板，因为后面需要
    ...
    if toLongNumber(coins) > toLongNumber(0) or #items > 0 then
        local callbackfunc = function(  )
            if CardSysManager:needDropCards("EGG RANKING") == true then
                -- 卡包开完消息 只在自己触发掉卡的时候监听 监听早了会被其他地方掉卡影响
                CardSysManager:doDropCards("EGG RANKING")
            end
...
return  InboxItem_holidayChallengeRankAward