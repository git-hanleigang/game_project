...
local LogDiningRoomActivity = class("LogDiningRoomActivity", NetworkLog)


function LogDiningRoomActivity:ctor()
    NetworkLog.ctor(self)
end

-- -- 新手引导打点
-- function LogDiningRoomActivity:sendGuideLog( guideName )
...
--[[
	pageName:推送宣传页=PushPage,关卡内推送界面=RoundPage,活动界面=ActivityPage
...
function LogDiningRoomActivity:sendPageLog( pageName,pageOpenType,entryType )
	...
	if enterType and entryName and entryOpen then
		...
		log_data.name = "Dining" .. self:formatActivityTime(gameData.p_start)
		log_data.day = gameData:getDay()
		log_data.rd = tonumber(gameData:getSequence())  -- 活动轮数
		log_data.ch = gameData:getCurrent() 			-- 第几关
		log_data.pn = pageName
		log_data.et = enterType
		log_data.en = entryName
		log_data.eo = entryOpen
		gL_logData:syncEventData("DiningPopup")
		gL_logData.p_data = log_data
		self:sendLogData()
	end
end
...
function LogDiningRoomActivity:formatActivityTime(activityTime)
