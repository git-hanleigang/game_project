...
local LogPokerActivity = class("LogPokerActivity", NetworkLog)

function LogPokerActivity:sendPokerPopupLog(_type, _pageName, _entryType)
    if not _type or not _pageName or not _entryType then
        return
    end

    local data = G_GetMgr(ACTIVITY_REF.Poker):getRunningData()
    if not data then
        return
    end
...
function LogPokerActivity:formatActivityTime(activityTime)
