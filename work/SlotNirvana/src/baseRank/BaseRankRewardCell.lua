...
function BaseRankRewardCell:initUI(csb_res, params)
    if csb_res then
        self:setCsbName(csb_res)
    end
    params = params or {}
    if params.maxLen ~= nil then
        self:setCoinMaxLen(params.maxLen)
    end
    if params.scale ~= nil then
        self:setRewardScale(params.scale)
    end
...
function BaseRankRewardCell:setRewards(items)
    ...
    for i, shopItemData in ipairs(items) do
        ...
        if shopItemUI ~= nil then
            shopItemUI:setIconTouchSwallowed(false)
