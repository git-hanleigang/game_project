...
function QuestCellTipRewards:initCsbNodes()
    ...
    --magic卡buff
    self.m_node_mythic_buff = self:findChild("node_mythic_buff")
end
...
function QuestCellTipRewards:resetMagicBuff()
    if not self.m_node_mythic_buff then
        return
    end
    self.m_node_mythic_buff:setVisible(self:hasCoins())
    ...
    if buffInfo then
        ...
        if not self.m_mythicBuff then
            local mythicBuff = G_GetMgr(G_REF.CardSpecialClan):createSpecialClanBuffNode()
            if mythicBuff then
                mythicBuff:updateBuffMultiple(nMuti)
                self.m_node_mythic_buff:addChild(mythicBuff)
                self.m_mythicBuff = mythicBuff
            end
        else
            self.m_mythicBuff:updateBuffMultiple(nMuti)
        end
    end
end
...
function QuestCellTipRewards:resetCoins()
    ...
    local stage_data = self.m_config:getStageData(self.phase_idx, self.stage_idx)
    local baseCoins = stage_data.p_coins
    -- magic卡册buff加成
    local buffmul = 1
    local buffInfo = globalData.buffConfigData:getBuffDataByType(BUFFTYPY.BUFFTYPE_SPECIALCLAN_QUEST)
    if buffInfo then
        local nMuti = tonumber(buffInfo.buffMultiple)
        buffmul = buffmul + nMuti * 0.01
    end
    ...
    baseCoins = baseCoins * buffmul
    self.lb_coins:setString(util_formatCoins(baseCoins, 8))
    ...
    if bl_hasDiscount then
        local discount = buffmul + self.m_config.p_discount * 0.01
        lastCoins = baseCoins / discount
    else
        if buffmul > 1 then
            self.lb_coins_base:setVisible(true)
            lastCoins = baseCoins / buffmul
        end
    end
    if lastCoins > 0 then
        height_percent = 0.73
    ...
    local panelCoinsWidth = node_pos.x + coins_width + edge_x
    if self.m_node_mythic_buff and buffmul > 1 then
        local buffNodePosX = self.m_node_mythic_buff:getPositionX()
        local world_pos = self.m_node_mythic_buff:getParent():convertToWorldSpace(cc.p(buffNodePosX, 0))
        local node_pos = self.panel_coins:convertToNodeSpace(world_pos)
        local width = node_pos.x + 35 + edge_x
        panelCoinsWidth = math.max(width, panelCoinsWidth)
    end
    self.panel_coins:setContentSize(panelCoinsWidth, panel_size.height)
end

function QuestCellTipRewards:resetPoints()
    if not self:hasCoins() then
        self.panel_coins:setVisible(false)
        return
    end
    ...
    local passData = G_GetMgr(ACTIVITY_REF.Quest):getRunningData():getPassData()
    local nMuti = passData:getPointMoreBuf()
    if nMuti > 0 then
        buffmul = buffmul + nMuti
    end
