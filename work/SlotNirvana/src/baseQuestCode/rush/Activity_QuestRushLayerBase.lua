...
local Activity_QuestRushLayerBase = class("Activity_QuestRushLayerBase", BaseLayer)
...
function Activity_QuestRushLayerBase:ctor()
    Activity_QuestRushLayerBase.super.ctor(self)
...
function Activity_QuestRushLayerBase:initDatas(_data)
...
function Activity_QuestRushLayerBase:initUI()
    Activity_QuestRushLayerBase.super.initUI(self)
...
function Activity_QuestRushLayerBase:hideEntranceNodes()
...
function Activity_QuestRushLayerBase:getCsbPath()
    ...
    release_print("Activity_QuestRushLayerBase:getCsbName rushType is " .. tostring(rushType))
    local csbPath = themeConfig[rushType].TASK_MAIN
    if not csbPath then
        printError("quest挑战 主面板资源没配置 " .. theme_name)
    end
    return csbPath
end

function Activity_QuestRushLayerBase:initCsbNodes()
...
function Activity_QuestRushLayerBase:initBgUI()
    -- 任务描述
    local rushType = G_GetMgr(ACTIVITY_REF.QuestRush):getRushType()
    if rushType == self.m_config.RUSH_TYPE.STAGE then
        self.lb_task:setString("PASS SLOT GAMES IN QUEST TO WIN PRIZES")
    elseif rushType == self.m_config.RUSH_TYPE.CHAPTER then
    ...
    if rushType == self.m_config.RUSH_TYPE.STAGE or rushType == self.m_config.RUSH_TYPE.CHAPTER then
        local diff = G_GetMgr(ACTIVITY_REF.QuestRush):getDifficulty()
        local diff_str = ""
        if diff <= 1 then
...
function Activity_QuestRushLayerBase:getItemPath()
    assert(false, "Activity_QuestRushLayerBase:getItemPath 需要子类指定文件路径")
end

-- init 奖励UI
function Activity_QuestRushLayerBase:initRewardsUI()
...
function Activity_QuestRushLayerBase:resetRewardPosition()
...
-- 刷新累积的进度数值
function Activity_QuestRushLayerBase:updateProcessNum(_bPre)
    ...
    if self.lb_curNum then
        ...
        util_scaleCoinLabGameLayerFromBgWidth(self.lb_curNum, width)
    end
...
-- 更新进度条
function Activity_QuestRushLayerBase:updateProgressUI(_layoutW)
...
-- 更新进度
function Activity_QuestRushLayerBase:updateProgress()
    if not self.m_layoutW then
        if self.m_isEntrance then
            self.m_layoutW = self:getLengthToComplete(false)
        else
...
function Activity_QuestRushLayerBase:updateItemsState(bl_isNew)
...
-- 刷新活动倒计时
function Activity_QuestRushLayerBase:updateTimer()
    -- 刷新倒计时
...
function Activity_QuestRushLayerBase:onKeyBack()
    self:closeUI(true)
end

function Activity_QuestRushLayerBase:onEnter()
    Activity_QuestRushLayerBase.super.onEnter(self)

    -- 活动结束事件
    gLobalNoticManager:addObserver(
        self,
        function(self, params)
            if params.name == ACTIVITY_REF.QuestRush then
                self:closeUI()
            end
        end,
    ...
    gLobalNoticManager:addObserver(
        self,
        function(self)
            if G_GetMgr(ACTIVITY_REF.QuestRush):getNeedRefreshProcess() then
                G_GetMgr(ACTIVITY_REF.QuestRush):setNeedRefreshProcess(false)
                self.m_layoutW = nil
...
function Activity_QuestRushLayerBase:closeUI(isNotify)
    ...
    Activity_QuestRushLayerBase.super.closeUI(self, callback)
end

-- 统一点击回调
function Activity_QuestRushLayerBase:clickFunc(sender)
...
-- 去quest主界面
function Activity_QuestRushLayerBase:goToQuestMainView()
    local questConfig = G_GetMgr(ACTIVITY_REF.Quest):getRunningData()
    if questConfig and questConfig.m_isQuestLobby then
        return
    end

    gLobalSendDataManager:getLogQuestActivity():sendQuestEntrySite("questRushToQuestMain")
    G_GetMgr(ACTIVITY_REF.Quest):showMainLayer()
end

-- 获取进度条最大的进度
function Activity_QuestRushLayerBase:getLengthToComplete(_bPre)
    -- 当前进度
...
-- 初始的最小进度
function Activity_QuestRushLayerBase:getProgShowMinWidth()
    return MIN_PRG_WIDTH
end

return Activity_QuestRushLayerBase
