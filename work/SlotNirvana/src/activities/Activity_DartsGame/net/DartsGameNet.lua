...
function DartsGameNet:sendStartGameReq(gameIdx)
    ...
    local successCallback = function (result)
        ...
        G_GetMgr(ACTIVITY_REF.DartsGame):getData():getGameDataBuyIndex(gameIdx):setGameStartData(result)
    ...
    self:sendActionMessage(ActionType.DartsGamePlay, reqData, successCallback, failedCallback)
end
...
function DartsGameNet:sendGetReeward(gameIdx)
    ...
    self:sendActionMessage(ActionType.DartsGameReward, reqData, successCallback, failedCallback)
end
...
function DartsGameNet:sendGameEnd(gameIdx)
    ...
    local successCallback = function (result)
        ...
        if cData then
            cData:removeDartsGameMail(gameIdx)
        end
        ...
        G_GetMgr(ACTIVITY_REF.DartsGame):getData():removeGameDataByIndex(gameIdx)
    end
    ...
    self:sendActionMessage(ActionType.DartsGameEnd, reqData, successCallback, failedCallback)
end
...
function DartsGameNet:goPurchase(_gameData)
    ...
    local localId = string.format("%s_%s", BUY_TYPE.DartsGame, idx)
    gLobalSaleManager:purchaseActivityGoods(localId, idx, BUY_TYPE.DartsGame, goodsInfo.goodsId, goodsInfo.goodsPrice, 0, 0, handler(self, self.buySuccess), handler(self, self.buyFailed))
end
