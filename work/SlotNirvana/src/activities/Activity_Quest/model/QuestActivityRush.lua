...
local QuestActivityRush = class("QuestActivityRush", BaseActivityData)

-- message QuestChallenge {
...
--     repeated QuestChallengePhase phaseResult = 6;//阶段数据
--     optional int64 taskId = 7; //任务类型
--     optional int32 difficulty = 8;//难度条件
-- }
function QuestActivityRush:parseData(_data)
    QuestActivityRush.super.parseData(self, _data)
...
-- }
function QuestActivityRush:parseRewardsData( _data, _dataEx )
    local base = 0
    local rewardsData = {}
    if _data then
...
-- 获取任务划分为几个阶段
function QuestActivityRush:getRushParts()
...
-- 获取任务对应阶段的起始条件
function QuestActivityRush:getBaseByIdx(_idx)
...
-- 获取任务对应阶段的完成条件
function QuestActivityRush:getConditionByIdx(_idx)
    if self.m_rewardsData[_idx] then
...
-- 获取总体任务完成条件
function QuestActivityRush:getRushCompleteCondition()
...
-- 获取老数据的data process
function QuestActivityRush:getOldDataProcess()
...
-- 获取老数据的 进度
function QuestActivityRush:getPreProcess()
...
-- 获取新数据的 process
function QuestActivityRush:getCurProcess()
    local condition = 0
...
-- 获取奖励 集合
function QuestActivityRush:getRewardsInfoList() 
    return self.m_rewardsData
end
-- 获取奖励 信息 _idx 
function QuestActivityRush:getRewardDataByIdx(_idx)
    return self.m_rewardsData[_idx] or {}
end

-- 更新老数据
function QuestActivityRush:resetOldData()
...
-- 活动类型
function QuestActivityRush:getRushType()
...
function QuestActivityRush:getDifficulty()
    return self.difficulty or 0
end

-- 当前任务进行到哪个阶段
function QuestActivityRush:getCurrentStep()
    return self.m_curStep or 1
end

function QuestActivityRush:getPreStep()
    return self.m_oldStep
end

return QuestActivityRush