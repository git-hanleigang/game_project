...
local QuestRankData = class("QuestRankData")
QuestRankData.p_rankUsers = nil
QuestRankData.p_myRank = nil
QuestRankData.p_rewards = nil
QuestRankData.p_seasons = nil
QuestRankData.p_prizePool = nil --金币池

function QuestRankData:ctor()
    self.p_rankUsers = {}
    self.p_rewards = {}
    self.p_seasons = {}
end

function QuestRankData:parseData(data)
    if not data then
        return
    end
...
--QuestRankUser
function QuestRankData:createRankUser(data)
    local rankUser = {
        p_rank = data.rank,
        p_name = data.name,
...
--QuestReward
function QuestRankData:createReward(data)
    local reward = {
        p_rank = data.minRank,
...
--QuestSeason
function QuestRankData:createSeason(data)
    local season = {
        p_rank = data.rank,
...
--获取自己在所有排名中的序号
function QuestRankData:getSelfRankIndex()
...
--获取排名
function QuestRankData:getRankIndex(udid)
...
--获取排行信息
function QuestRankData:getRankConfig(index)
    if index == 2 then
...
--获取自己排行信息
function QuestRankData:getMyRankConfig()
    return self.p_myRank
end

return QuestRankData
