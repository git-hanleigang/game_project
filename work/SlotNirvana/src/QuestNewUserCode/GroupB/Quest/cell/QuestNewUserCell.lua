...
function QuestNewUserCell:initCsbNodes()
    ...
    self.node_gift = self:findChild("node_gift")
...
function QuestNewUserCell:initGift()
    local act_data = G_GetMgr(ACTIVITY_REF.Quest):getRunningData()
    if not act_data then
        return
    end
    local phase_data = act_data:getPhaseData(self.m_curPhase)
    if phase_data and phase_data.p_stages and self.m_curStage >= #phase_data.p_stages then
        return
    end
    self.cell_gift = util_createView(QUEST_CODE_PATH.QuestGift, self.m_curPhase, self.m_curStage)
    if self.cell_gift then
        self.cell_gift:addTo(self.node_gift)
    end
end
