...
local QuestNewUserMainViewC = class("QuestNewUserMainViewC", BaseActivityMainLayer)
...
function QuestNewUserMainViewC:ctor()
    QuestNewUserMainViewC.super.ctor(self)
...
function QuestNewUserMainViewC:getQuestData()
...
--初始化数据
function QuestNewUserMainViewC:initDatas()
...
function QuestNewUserMainViewC:initCsbNodes()
...
function QuestNewUserMainViewC:initLogo()
...
function QuestNewUserMainViewC:initTitle()
...
function QuestNewUserMainViewC:onShowedCallFunc()
    self:runCsbAction("idle", true, nil, 60)
end

function QuestNewUserMainViewC:initView()
    self:initLogo()
    self:initTitle()
...
-- 更新顶部的阶段奖励
function QuestNewUserMainViewC:updateTitle()
    if self.item_title then
        self.item_title:updatePhaseReward()
    end
end

function QuestNewUserMainViewC:initScrollBackground()
    --背景条
    local nodeInfoList = {}
    ...
    self.m_questMapControl:updateMap(self.m_questMapScroll:getCurrentOffset())
end

-- 初始化宝箱
function QuestNewUserMainViewC:initBox()
...
-- 初始化单个宝箱
function QuestNewUserMainViewC:initBoxByIndex(phase_idx, stage_idx)
...
function QuestNewUserMainViewC:onEnter()
    QuestNewUserMainViewC.super.onEnter(self)
...
function QuestNewUserMainViewC:showBoxOpen()
...
function QuestNewUserMainViewC:showRewardLayer()
...
function QuestNewUserMainViewC:changeStageHandler()
...
-- 新手quest完成 结束弹板
function QuestNewUserMainViewC:showOverLayer()
...
function QuestNewUserMainViewC:onExit()
    QuestNewUserMainViewC.super.onExit(self)
    if self.m_config ~= nil then
        self.m_config.m_isQuestLobby = false
    end
end

function QuestNewUserMainViewC:checkResetMapPos()
    local questConfig = self:getQuestData()
...
--获得关卡节点宽度
function QuestNewUserMainViewC:getCellWidth()
...
--初始化箭头
function QuestNewUserMainViewC:initArrow()
    if not self.m_config then
        return
    end
...
-- 获得当前地图中的位置
function QuestNewUserMainViewC:getStartPosX()
...
function QuestNewUserMainViewC:initQuestLayer(startStageIndex)
    self.m_questCellList = {}
...
-- 初始化关卡Cell
function QuestNewUserMainViewC:initLevelCells(phasesList, curCellIndex)
...
function QuestNewUserMainViewC:createLevelCell(stageData)
...
function QuestNewUserMainViewC:updateLevelCell()
    local phasesList = self.m_config.p_phases
...
function QuestNewUserMainViewC:onKeyBack()
    --引导期间不能点击返回键
    local questConfig = self:getQuestData()
    if questConfig and questConfig.p_expireAt then
        --不是第一关 可能是清除本地数据
...
function QuestNewUserMainViewC:checkShowWheelGuide()
    local questConfig = self:getQuestData()
    if questConfig and questConfig.p_expireAt then
        local isWheelGuide = gLobalDataManager:getBoolByField("quest_wheelGuide" .. questConfig.p_expireAt, true)
        --不是第一关 可能是清除本地数据
...
--默认按钮监听回调
function QuestNewUserMainViewC:clickFunc(sender)
...
function QuestNewUserMainViewC:closeUI()
    ...
    QuestNewUserMainViewC.super.closeUI(
        self,
        function()
...
-- 滚动到轮盘位置
function QuestNewUserMainViewC:moveTo(currentOffset, targetOffset, moveDir, overFunc)
...
-- TODO:
function QuestNewUserMainViewC:getOffsetByIndex(index)
...
function QuestNewUserMainViewC:getBoxOffsetByIndex(index)
    local cellWidth = self:getCellWidth()
...
function QuestNewUserMainViewC:getZOrderByIndex(index)
    local maxStageCount = self.m_config:getStageCount()
    return 10 + (maxStageCount - index)
end

--解锁
function QuestNewUserMainViewC:questCellUnlock(lastIndex)
